...
| `make test-ral-pipeline-runtime WIRED=...` | the real `ral_pipeline_test` command creates a fresh offscreen Vulkan/RAL backend and proves 100-way layout sharing/teardown, graphics draw/readback, exact compute SSBO output, a binary-semaphore graphics acquire over the exact `baseMip=2/mipCount=1/baseLayer=0/layerCount=1` range before a portable restricted view samples coarse green instead of mip0 red, pipeline-cache roundtrip and zero live diagnostic resources before clean process shutdown |
...
| `imgui_tool_policy_contract` | `BUILD_TESTING=ON`, Ninja | Wired tooling/build | exact five-source Dear ImGui adapter cohort compiles only in `wired_profile_imgui`, never inherits `HEADLESS`, and has only the isolated host-contract link consumer rather than shipping GUI/headless | contract | `host-only`, `infrastructure`, `owner.ral`, `owner.tools`, `failure.build-graph`, `feature.imgui` |
