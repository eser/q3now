...
clear_pat=re.compile(r"ral-storage-clear schema=1 map=([^ ]+) family=(cull|forward-plus|hdr-histogram) bytes=([1-9][0-9]*) visibility=transfer-to-compute")
...
for map_name in maps:
 ...
 for family,expected in (("cull",4),("forward-plus",None),("hdr-histogram",1024)):
...
if len(clear_found)!=6:raise SystemExit(f"FAIL storage clear unexpected receipt set {sorted(clear_found)}")
...
with open(sys.argv[1],"w") as out:
 for map_name in ("arena1","arena17"):
  ...
  rows += [("INFO","renderer.ral",f"ral-storage-clear schema=1 map={map_name} family={family} bytes={size} visibility=transfer-to-compute") for family,size in (("cull",4),("forward-plus",4096),("hdr-histogram",1024))]
