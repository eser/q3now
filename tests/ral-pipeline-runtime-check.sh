...
claimed=("RAL swapchain ready:","RAL swapchain recreate:","RAL swapchain frame:","ral_pipeline_test:","===== RAL dump ","Vulkan backend ready:","===== RAL pipeline test ","  pipelineCache present:","  layout-cache share:","  draw:","  compute:","  residency acquire:","  residency view:","  pipeline cache:","  teardown:","===== end RAL pipeline test =====","Ral_DestroyBackend:","===== end RAL dump =====","Q0_RAL_PIPELINE_","----- Server Shutdown ","==== ShutdownGame ====")
for value in vals:
 parts=re.split(r"[\r\n]",value)
...
def exact(prefix,pattern,sev="INFO",cat="renderer.ral",count=1):
 ...
 return found
swapchains=exact("RAL swapchain ready:",r"RAL swapchain ready: recreate=1 extent=[1-9][0-9]*x[1-9][0-9]* images=[1-9][0-9]* format=[0-9]+ colorSpace=[0-9]+ presentMode=[0-9]+ usage=0x[0-9a-f]+ renderable=[1-9][0-9]*",count=2)
for _,_,value in swapchains:
 sm=re.fullmatch(r"RAL swapchain ready: recreate=([01]) extent=([1-9][0-9]*)x([1-9][0-9]*) images=([1-9][0-9]*) format=([0-9]+) colorSpace=([0-9]+) presentMode=([0-9]+) usage=0x([0-9a-f]+) renderable=([1-9][0-9]*)",value)
 if not sm or int(sm.group(4))!=int(sm.group(9)) or (int(sm.group(8),16)&0x4)==0:raise SystemExit(f"FAIL ral-pipeline non-renderable/non-color swapchain receipt: {value}")
...
def swapchain_vector(value):
 m=re.fullmatch(r"RAL swapchain ready: recreate=1 extent=([1-9][0-9]*)x([1-9][0-9]*) images=([1-9][0-9]*) format=([0-9]+) colorSpace=([0-9]+) presentMode=([0-9]+) usage=0x([0-9a-f]+) renderable=([1-9][0-9]*)",value)
...
order=(first[0],receipt[0],dump[0],backend[0],start[0],present[0],share_create[0],share_destroy[0],draw[0],compute[0],acquire[0],residency[0],cache[0],teardown[0],end[0],destroy[0],dump_end[0],recreate[0][0],swapchains[0][0],recreate[1][0],frames[0][0],recreate[2][0],swapchains[1][0],recreate[3][0],frames[1][0],view[0][0],complete[0],shutdown[0],shutdown_game[0])
...
if mode=="old-claim":R.insert(2,row("INFO","renderer.ral","Rendering is now exclusively on the legacy VkPipeline path."))
 ...
 i=findall("RAL swapchain ready:")[1];R[i]["msg"]=R[i]["msg"].replace("extent=640x480","extent=800x600")
...
if [ "${1:-}" = --self-test ];then
 ...
 defects=(old-claim missing duplicate suffix metadata smuggle layout destroy draw-grey draw-clear compute acquire-missing acquire-range acquire-failed residency-missing residency-wrong residency-additive cache teardown vuid swapchain-missing swapchain-renderable swapchain-vector swapchain-drift swapchain-usage recreate-order frame-missing frame-stale frame-image-oob frame-prepare view-missing order backend-fail manifest result bootstrap)
