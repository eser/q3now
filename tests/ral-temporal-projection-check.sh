...
for capture_n,(s,c,main,motion,history,submit,content) in enumerate(zip(rsm,rcm,sm,gm,hm,resolve_submits,resolve_contents)):
 ...
 if capture_w!=min(extent_w,core_w+8) or capture_h!=min(extent_h,core_h+8) or capture_x!=(extent_w-capture_w)//2 or capture_y!=(extent_h-capture_h)//2:raise SystemExit("FAIL temporal resolve apron geometry")
 if int(c.group(36))!=core_w*core_h or int(c.group(37))!=int(c.group(38)) or c.group(38)!=c.group(39) or int(c.group(40))<=0 or int(c.group(41))<=0 or int(c.group(40))>int(c.group(39)) or int(c.group(41))>int(c.group(38)):raise SystemExit("FAIL temporal resolve oracle accounting")
...
for n,(frame,token,slot,serial,l0,l1) in enumerate(gpu):
 ...
 R += [row("INFO","renderer.ral",f"temporal-resolve-content schema=1 token={token} frame={frame} previous={frame-1} content={502+n} world=0 extent=1280x720 topology=3 plan=4 scene=5 history=3:{1-n} motion-materialization=10 motion-target=8 motion-layout=9 table=11 prior-producer={prior_producer} prior-token={prior_token} prior-frame={prior_frame} prior-content={prior_content} prior-scene=5 prior-target=8 prior-resolve-owner={prior_owner} prior-store-owner=13 prior-slot={prior_slot} prior-frame-count=2 target=8 owner=12 slot={slot} serial={serial} depth=4 segments=2 written=1 invalidated=1 sequence={l0}:{l1}:3 capture=508,228,264x264 core=512,232,256x256 outer-submit=1 content-submit=1 fence=1 core-pixels=65536 eligible=5536 accepted=5536 accepted-match=5536 accepted-influence=4096 accepted-motion=2048 fallback=59477/59477 invalid=59477/59477 zero=0/0 unsupported=523 nonfinite=0 validity=64160:5536:0 planes=1 mismatches=0 hashes={hashes} ready=1")]
...
elif mode=="resolve-influence":R[find("temporal-resolve-content",2)]["msg"]=R[find("temporal-resolve-content",2)]["msg"].replace("accepted-influence=4096","accepted-influence=0")
...
elif mode=="resolve-validity":R[find("temporal-resolve-content",2)]["msg"]=R[find("temporal-resolve-content",2)]["msg"].replace("validity=64160:5536:0","validity=64159:5536:1")
...
python3 "$TIMEOUT_RUNNER" --timeout 120 --kill-after 15 --cwd "$RUN" --stdout "$STDOUT" -- "$RUN/wired" +set fs_basepath "$HOME_DIR" +set fs_homepath "$HOME_DIR" +set fs_game base +set vm_game 0 +set vm_cgame 0 +set char h5a_fixture +set skin default +set sv_cheats 1 +set sv_pure 0 +set com_automated 1 +set com_noHardReboot 1 +set s_initsound 0 +set r_fullscreen 0 +set r_mode 3 +set r_vkValidate 0 +set r_entitySSBO 1 +set r_temporalInputTest 0 +set r_bloom 0 +set r_ssao 0 +set r_smaa 0 +set r_forwardPlus 0 +set r_drawSunRays 0 +set r_shadows 0 +set r_depthFade 0 +set r_lens 0 +set r_gpuDecals 0 +set r_particles 0 +set r_atmosphericGPU 0 +set log_severity DEBUG +set log_file_severity DEBUG +set log_file_mode overwrite_synced +exec ral-temporal-projection.cfg
