...
# ── per-build stamp (monotonic counter + timestamp) ────────────────────────────
...
# write both into a GENERATED HEADER that every component includes (via
# wired/wired_build_stamp.h). The header route (not -D) is deliberate: it refreshes
# every build with NO cmake reconfigure, sidesteps the windres/Ninja spaced-string
# -D escaping trap entirely (the value is plain C source), and reaches the WASI
# wasm builds for free (they compile the same TU that includes it). The header is
# only REWRITTEN when the values change so unrelated rebuilds don't churn it.
...
_build-stamp:
	@n=$$(cat .build_number 2>/dev/null || echo 0); \
	 ...
	 { echo "// Generated by 'make' (_build-stamp). DO NOT EDIT; not committed."; \
	   ...
	   echo "#define WIRED_BUILD_DATE \"$$ts\""; } > $$tmp; \
	 if ! cmp -s $$tmp $(WIRED_BUILD_STAMP_HDR) 2>/dev/null; then \
	   mv $$tmp $(WIRED_BUILD_STAMP_HDR); \
	 else rm -f $$tmp; fi
...
check: create-packs
	...
	@ls $(MODULE_DIR)/vm/gamecl.wasm  > /dev/null 2>&1 && echo "  gamecl VM:    OK" || echo "  gamecl VM:    MISSING (wasi-sdk not found?)"
	@ls $(MODULE_DIR)/vm/gamesv.wasm > /dev/null 2>&1 && echo "  gamesv VM:   OK" || echo "  gamesv VM:   MISSING (wasi-sdk not found?)"
	@ls $(MODULE_DIR)/gamecl$(GAME_ARCH).*    > /dev/null 2>&1 && echo "  gamecl native: OK" || echo "  gamecl native: MISSING"
	@ls $(MODULE_DIR)/gamesv$(GAME_ARCH).*  > /dev/null 2>&1 && echo "  gamesv native: OK" || echo "  gamesv native: MISSING"
