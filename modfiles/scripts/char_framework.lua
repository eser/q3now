...
-- Loaded by sv_lua.c at init; installs wired.load_character and wired.load_bot.
--
-- wired.load_character(name, skill_norm):
...
local function log(msg)
    if wired and type(wired.print) == "function" then
        wired.print(msg)
    else
        print(msg)
    end
...
function wired.load_character(name, _skill_norm)
    local base = wired.load("characters/_archetypes/_base/main")
    if type(base) ~= "table" then
        error("characters/_archetypes/_base/main.lua must return a table", 0)
    end

    local char = wired.load("characters/" .. name .. "/main")
    ...
    local ok_arch, arch = pcall(function() return wired.load(arch_path) end)
...
function wired.load_bot(name)
    local base = wired.load("characters/_archetypes/_base/bot/main")
    if type(base) ~= "table" then
    ...
    local ok_char, main_char = pcall(function()
        return wired.load("characters/" .. name .. "/main")
    end)
    ...
    local ok_arch, arch_bot = pcall(function()
        return wired.load("characters/_archetypes/" .. archetype_name .. "/bot/main")
    end)
    ...
    local ok_c, charBot = pcall(function() return wired.load(charPath) end)
    ...
    local chats_mod
    do
        local ok, loaded = pcall(wired.load, "characters/" .. name .. "/bot/chats")
        if ok and type(loaded) == "table" then
            chats_mod = loaded
        else
            local ok2, base_chats = pcall(wired.load, "characters/_archetypes/_base/bot/chats")
