...
static void R_PerformanceCounters( void ) {
	...
	if (r_speeds->integer == 1) {
		R_LOG( rch_cmd, SEV_INFO, "%i/%i shaders/surfs %i leafs %i verts %i/%i tris %.2f mtex %.2f dc\n",
...
void RE_BeginFrame( stereoFrame_t stereoFrame ) {
	...
	if ( (stereoFrame == STEREO_LEFT || stereoFrame == STEREO_CENTER)
		...
		gls.windowWidth = (int)s_pendingPresentationChange.presentationWidth;
		gls.windowHeight = (int)s_pendingPresentationChange.presentationHeight;
	...
	else
	{
		...
		{
			if ( r_anaglyphMode->modificationCount != s_anaglyph_mod )
			{
				s_anaglyph_mod = r_anaglyphMode->modificationCount;
				clrcmd = R_GetCommandBuffer( sizeof( *clrcmd ) );
				if ( clrcmd ) {
					memset( clrcmd, 0, sizeof( *clrcmd ) );
...
#ifdef USE_FBO
		{
			static int s_multisample_mod = -1, s_hdr_mod = -1;
			if ( r_ext_multisample->modificationCount != s_multisample_mod
			     || r_hdr->modificationCount != s_hdr_mod ) {
				s_multisample_mod = r_ext_multisample->modificationCount;
				s_hdr_mod = r_hdr->modificationCount;
				QGL_InitFBO();
			}
		}
#endif
