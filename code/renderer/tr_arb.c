...
static void MSDF_GLSL_Init( void )
{
	...
	if ( !ok ) {
		...
		R_LOG( rch_gl, SEV_WARN, "MSDF GLSL VP: %s\n", log );
		qglDeleteShader( vs );
		return;
	}
	...
	if ( !ok ) {
		...
		R_LOG( rch_gl, SEV_WARN, "MSDF GLSL FP: %s\n", log );
		qglDeleteShader( vs );
		qglDeleteShader( fs );
		return;
	}
	...
	if ( !ok ) {
		...
		R_LOG( rch_gl, SEV_WARN, "MSDF GLSL link: %s\n", log );
	...
	R_LOG( rch_gl, SEV_INFO, "...MSDF GLSL fallback ready\n" );
}
...
qboolean ARB_CompileProgram( programType ptype, const char *text, GLuint program )
{
	...
	if ( (errCode = qglGetError()) != GL_NO_ERROR || errorPos != -1 )
	{
		...
		{
			R_LOG( rch_gl, SEV_INFO, S_COLOR_YELLOW "%s Compile Error(%i,%i): %s\n" S_COLOR_CYAN "%s\n", (ptype == Fragment) ? "FP" : "VP",
...
static qboolean FBO_Create( frameBuffer_t *fb, GLsizei width, GLsizei height, qboolean depthStencil, GLint *outFormat, GLint *outType )
{
	...
	{
		R_LOG( rch_gl, SEV_INFO, "Failed to create %s (%s:%s) FBO (status %s, error %s)\n",
...
static qboolean FBO_CreateMS( frameBuffer_t *fb, int width, int height )
{
	...
	while ( nSamples > 0 ) {
		...
		if ( (int)qglGetError() == GL_INVALID_VALUE/* != GL_NO_ERROR */ ) {
			R_LOG( rch_gl, SEV_INFO, "...%ix MSAA is not available\n", nSamples );
			nSamples -= 2;
		} else {
			R_LOG( rch_gl, SEV_INFO, "...using %ix MSAA\n", nSamples );
			break;
		}
	}
	...
	if ( fboStatus != GL_FRAMEBUFFER_COMPLETE )
	{
		R_LOG( rch_gl, SEV_WARN, "Failed to create MS FBO (status %s, error %s)\n", glDefToStr( fboStatus ), glDefToStr( (int)qglGetError() ) );
		FBO_Clean( fb );
		return qfalse;
	}
...
static qboolean FBO_CreateBloom( void )
{
	...
	{
		R_LOG( rch_gl, SEV_WARN, "...not enough texture units (%i) for %i-pass bloom\n",
	...
	R_LOG( rch_gl, SEV_INFO, "...%i bloom passes\n", fboBloomPasses );
...
qboolean FBO_Bloom( const float gamma, const float obScale, qboolean finalStage )
{
	...
	if ( !fboBloomInited )
	{
		...
		{
			R_LOG( rch_gl, SEV_WARN, "...error creating framebuffers for bloom\n" );
			ri.Cvar_Set( "r_bloom", "1" );
			FBO_CleanBloom();
			return qfalse;
		}

		R_LOG( rch_gl, SEV_INFO, "...bloom framebuffers created\n" );
	}
...
void QGL_SetRenderScale( qboolean verbose )
{
	...
	if ( !r_fbo->integer )
	{
		...
		{
			R_LOG( rch_gl, SEV_INFO, "...ignoring \r_renderScale due to disabled FBO\n" );
		}
		return;
	}
...
void QGL_InitFBO( void )
{
	...
	if ( r_fbo->integer && ( !qglGenProgramsARB || !qglGenFramebuffers ) )
		R_LOG( rch_gl, SEV_WARN, "...FBO is not available\n" );
	...
	if ( result )
	{
		fboEnabled = qtrue;
		FBO_BindMain();
		R_LOG( rch_gl, SEV_INFO, "...using %s (%s:%s) FBO\n", glDefToStr( fboInternalFormat ),
