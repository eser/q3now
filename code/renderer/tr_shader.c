...
static qboolean ParseStage( ComParser *parser, shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		// animMapClamp <frequency> <image1> .... <imageN>  -- clamp to last frame instead of looping
		//
		...
		//
			...
			if ( !Q_stricmp( token, "wave" ) ) {
				ParseWaveForm( parser, text, &stage->rgbWave );
		...
		//
			...
			if ( !Q_stricmp( token, "wave" ) ) {
				ParseWaveForm( parser, text, &stage->alphaWave );
		...
		//
		// zFadeBounds <near> <far>
		//    Distance-based stage fading. The stage fades in between
		//    <near> and <far> world units from the view origin.
		//
...
static qboolean CollapseMultitexture( shaderStage_t *st0, shaderStage_t *st1, int num_stages ) {
	...
	// when the two stages produce non-white colors (any rgbGen/alphaGen
	// other than IDENTITY/SKIP) the per-vertex color array applied to
	// the collapsed pair only carries stage 0's modulation - stage 1's
	// intended contribution is silently lost. so refuse to collapse
	// unless we are sure stage 1 contributes pure white per-vertex.
	// example shader broken without this test: textures/sfx/diamond2cjumppad
	// (the ring would pulse in and out instead of only out)
	if ( st1->rgbGen != CGEN_IDENTITY && st1->rgbGen != CGEN_IDENTITY_LIGHTING ) {
		return qfalse;
	}
	if ( st1->alphaGen != AGEN_IDENTITY && st1->alphaGen != AGEN_SKIP ) {
		return qfalse;
	}

	if ( st0->rgbGen == CGEN_WAVEFORM )
	{
		if ( memcmp( &st0->rgbWave, &st1->rgbWave, sizeof( stages[0].rgbWave )  ) != 0 )
		{
			return qfalse;
		}
	}
	...
	{
		if ( memcmp( &st0->alphaWave, &st1->alphaWave, sizeof( stages[0].alphaWave )  ) != 0 )
		{
			return qfalse;
		}
	}
...
static int R_CompareShadersForSort( const shader_t *a, const shader_t *b )
{
	...
	if ( a->sort > b->sort ) return 1;
	// within the same sort bucket, polygon-offset surfaces must come last
	// otherwise decals can flicker and stack in inconsistent orders
	if ( a->polygonOffset != b->polygonOffset )
		return (int)a->polygonOffset - (int)b->polygonOffset;
	// finally use cullType so two-sided surfaces sit consistently relative
	// to single-sided ones inside the same transparent class
...
shader_t *R_FindShader( const char *name, int lightmapIndex, qboolean mipRawImage ) {
	...
	//
	{
		...
		if ( !image ) {
			// q3now meta-remap fallback: before defaulting to the pink
			// missing-texture shader, ask the active map's remap table
			// whether `name` has a substitute. Recursion-guarded via
			// R_PushNullRemap / R_PopRemap so a remap target that is
			// itself missing falls through to default rather than
			// looping. Only effective when SV_SpawnServer has populated
			// an active remap; no-op on stock content.
