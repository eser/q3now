...
/* entityState.generic1 metadata for an EV_MISSILE_MISS generated by fuse or
 * scripted detonation rather than a collision surface. */
#define WIRED_MISSILE_EVENT_FREE_AIR 0x00000001
...
// Coarse, wire-safe material response carried by hitscan impact events.
// Value 0 is the backwards-compatible fallback for old demos/servers and
// surfaces without an authored response flag.
typedef enum {
	HITSCAN_IMPACT_DEFAULT = 0,
	HITSCAN_IMPACT_METAL,
	HITSCAN_IMPACT_DUST,
	HITSCAN_IMPACT_MATERIAL_COUNT
} hitscanImpactMaterial_t;

static ID_INLINE hitscanImpactMaterial_t BG_HitscanImpactMaterialForSurfaceFlags( int surfaceFlags ) {
	if ( surfaceFlags & SURF_METALSTEPS ) {
		return HITSCAN_IMPACT_METAL;
	}
	if ( surfaceFlags & SURF_DUST ) {
		return HITSCAN_IMPACT_DUST;
	}
	return HITSCAN_IMPACT_DEFAULT;
}
