...
qboolean	G_TryPushingEntity( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
	...
	if (pushed_p > &pushed[MAX_GENTITIES]) {
		Com_Terminate( TERM_CLIENT_DROP, "pushed_p > &pushed[MAX_GENTITIES]" );
	}
	pushed_p->ent = check;
...
================
*/
void Q3_MatchTeam( gentity_t *teamLeader, int moverState, int time ) {
	gentity_t		*member;

	for ( member = teamLeader ; member ; member = member->teamchain ) {
		Q3_SetMoverState( member, moverState, time );
	}
}
...
================
*/
void Q3_ReturnToPos1( gentity_t *ent ) {
	Q3_MatchTeam( ent, MOVER_2TO1, level.time );
...
void Q3_Reached_BinaryMover( gentity_t *ent ) {
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		// return to pos1 after a delay; wait < 0 means stay open forever
		if ( ent->wait >= 0 ) {
			ent->think = Q3_ReturnToPos1;
		...
		Com_Terminate( TERM_CLIENT_DROP, "Q3_Reached_BinaryMover: bad moverState" );
	}
}
...
void Q3_Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if ( ent->flags & FL_TEAMMEMBER ) {
		Q3_Use_BinaryMover( ent->teammaster, other, activator );
		return;
	}
	...
	// if all the way up, just delay before coming down; wait < 0 means stay open forever
	...
	if ( ent->moverState == MOVER_2TO1 ) {
		...
		Q3_MatchTeam( ent, MOVER_1TO2, level.time - ( total - partial ) );
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		Q3_MatchTeam( ent, MOVER_2TO1, level.time - ( total - partial ) );
...
void Q3_InitMover( gentity_t *ent ) {
	...
	ent->use = Q3_Use_BinaryMover;
	ent->reached = Q3_Reached_BinaryMover;
...
/*
================
Q3_Touch_DoorTriggerSpectator
================
*/
static void Q3_Touch_DoorTriggerSpectator( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	int axis;
	float doorMin, doorMax;
	vec3_t origin;

	axis = ent->count;
	// the constants below relate to constants in Q3_Think_SpawnNewDoorTrigger()
	...
	TeleportPlayer(other, origin, tv(10000000.0, 0, 0), 400);
}

/*
================
Q3_Touch_DoorTrigger
================
*/
void Q3_Touch_DoorTrigger( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	if ( other->client && other->client->sess.sessionTeam == TEAM_SPECTATOR ) {
		...
		if ( ent->parent->moverState != MOVER_1TO2 &&
			...
			Q3_Touch_DoorTriggerSpectator( ent, other, trace );
		}
	}
	else if ( ent->parent->moverState != MOVER_1TO2 ) {
		if ( ent->parent->use )
			ent->parent->use( ent->parent, ent, other );
		else
			Q3_Use_BinaryMover( ent->parent, ent, other );
	}
}
...
void Q3_Think_SpawnNewDoorTrigger( gentity_t *ent ) {
	...
	/* Q1 team doors: only the master creates the shared trigger.
	   Members get FL_TEAMMEMBER after SP_ returns, so by the time
	   this think fires the flag is already set. */
	if ( ent->flags & FL_TEAMMEMBER ) {
		return;
	}
	...
	other->touch = Q3_Touch_DoorTrigger;
	// remember the thinnest axis
	other->count = best;
	trap_LinkEntity (other);

	Q3_MatchTeam( ent, ent->moverState, level.time );
}

void Q3_Think_MatchTeam( gentity_t *ent ) {
	Q3_MatchTeam( ent, ent->moverState, level.time );
}
...
void SP_q3_func_door (gentity_t *ent) {
	...
	ent->sound1to2 = ent->sound2to1 = G_SoundIndex("sound/movers/doors/dr1_strt.opus");
	ent->soundPos1 = ent->soundPos2 = G_SoundIndex("sound/movers/doors/dr1_end.opus");
...
/*
==============
Q3_Touch_PlatCenterTrigger

If the plat is at the bottom position, start it going up
===============
*/
void Q3_Touch_PlatCenterTrigger(gentity_t *ent, gentity_t *other, trace_t *trace ) {
	if ( !other->client ) {
		return;
	}
...
void SP_q3_func_plat (gentity_t *ent) {
	...
	ent->sound1to2 = ent->sound2to1 = G_SoundIndex("sound/movers/plats/pt1_strt.opus");
	ent->soundPos1 = ent->soundPos2 = G_SoundIndex("sound/movers/plats/pt1_end.opus");
...
/*
===============
Q3_Think_BeginMoving

The wait time at a corner has completed, so start moving again
===============
*/
void Q3_Think_BeginMoving( gentity_t *ent ) {
...
===============
*/
void Q3_Reached_Train( gentity_t *ent ) {
	gentity_t		*next;
	float			speed;
	vec3_t			move;
	float			length;
	...
	if ( next->wait ) {
		...
		ent->think = Q3_Think_BeginMoving;
		ent->s.pos.trType = TR_STATIONARY;
	}
}
...
/*
===============
Q3_Think_SetupTrainTargets

Link all the corners together
===============
*/
void Q3_Think_SetupTrainTargets( gentity_t *ent ) {
...
"light"		constantLight radius
*/
void SP_q3_func_train (gentity_t *self) {
	...
	self->reached = Q3_Reached_Train;

	// start trains on the second frame, to make sure their targets have had
	// a chance to spawn
	self->nextthink = level.time + FRAMETIME;
	self->think = Q3_Think_SetupTrainTargets;
}
