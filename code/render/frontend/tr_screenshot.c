...
static int ScreenshotCaseCompare( const char *left, const char *right ) {
	for ( ;; ++left, ++right ) {
...
static void ScreenshotCopy( char *target, const char *source, size_t size ) {
	if ( !target || !size ) return;
	(void)snprintf( target, size, "%s", source ? source : "" );
}

static int ScreenshotFormat( char *target, size_t size, const char *format, ... ) {
	int result;
	va_list args;
	va_start( args, format );
	result = vsnprintf( target, size, format, args );
	va_end( args );
	if ( size ) target[size - 1u] = '\0';
	return result;
}

#define Q_stricmp ScreenshotCaseCompare
#define Q_strncpyz ScreenshotCopy
#define Com_sprintf ScreenshotFormat
