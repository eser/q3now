...
fn main_1() {
    ...
    let _e477 = c.sizeStart;
    let _e479 = p_3.sizeJitterPick;
    effectiveStart = (_e477 + _e479);
    let _e481 = effectiveStart;
    let _e483 = c.sizeEnd;
    let _e485 = p_3.age;
    size = mix(_e481, _e483, _e485);
    let _e488 = c.sizeParm;
    param_8 = _e488;
    let _e489 = parmIsUnset_u0028_struct_u002d_ParticleParm_u002d_i1_u002d_i1_u002d_f1_u002d_f1_u002d_f1_u002d_f1_u002d_f1_u002d_f1_u002d_f1_u005b_8_u005d_1_u003b((&param_8));
    if !(_e489) {
        let _e492 = c.sizeParm;
        param_9 = _e492;
        let _e494 = p_3.age;
        param_10 = _e494;
        let _e496 = p_3.sizeJitterPick;
        param_11 = _e496;
        let _e497 = parmEval_u0028_struct_u002d_ParticleParm_u002d_i1_u002d_i1_u002d_f1_u002d_f1_u002d_f1_u002d_f1_u002d_f1_u002d_f1_u002d_f1_u005b_8_u005d_1_u003b_f1_u003b_f1_u003b((&param_9), (&param_10), (&param_11));
        size = _e497;
    }
    let _e499 = p_3.pos;
    let _e501 = unnamed_1.viewLeft;
    let _e503 = sx;
    let _e504 = size;
    let _e509 = unnamed_1.viewUp;
    let _e511 = uy;
    let _e512 = size;
    worldPos = ((_e499 + (_e501.xyz * (_e503 * _e504))) + (_e509.xyz * (_e511 * _e512)));
    let _e517 = unnamed_1.mvp;
    let _e518 = worldPos;
    unnamed.gl_Position = (_e517 * vec4<f32>(_e518.x, _e518.y, _e518.z, 1f));
    let _e525 = vertInQuad;
    indexable_2 = array<vec2<f32>, 6>(vec2<f32>(0f, 1f), vec2<f32>(0f, 0f), vec2<f32>(1f, 1f), vec2<f32>(1f, 1f), vec2<f32>(0f, 0f), vec2<f32>(1f, 0f));
    let _e527 = indexable_2[_e525];
    fragUV = _e527;
    let _e528 = color;
    fragColor = _e528;
    let _e530 = p_3.classHandle;
    let _e532 = c.colorDomain;
    particleClassHandle = (_e530 | (_e532 << bitcast<u32>(31u)));
    let _e537 = c.frameCount;
    if (_e537 > 1u) {
        let _e540 = p_3.age;
        let _e542 = c.frameCount;
        fr = (_e540 * f32(_e542));
        let _e546 = c.frameCount;
        last = (_e546 - 1u);
        let _e548 = fr;
        let _e550 = last;
        frame0_ = u32(clamp(floor(_e548), 0f, f32(_e550)));
        let _e554 = frame0_;
        let _e556 = last;
        frame1_ = min((_e554 + 1u), _e556);
        let _e558 = frame0_;
        let _e561 = c.frameSlots[_e558];
        frameSlot0_ = _e561;
        let _e562 = frame1_;
        let _e565 = c.frameSlots[_e562];
        frameSlot1_ = _e565;
        let _e567 = c.frameBlend;
        if (_e567 != 0u) {
            let _e569 = fr;
            local = fract(_e569);
        } else {
