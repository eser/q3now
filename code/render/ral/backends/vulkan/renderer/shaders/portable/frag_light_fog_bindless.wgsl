...
fn main_1() {
    ...
    let _e81 = unnamed.packed_indices[0i][1u];
    let _e87 = unnamed.packed_indices[0i][1u];
    ...
    Np = _e95;
    param_1 = 0u;
    let _e96 = parallax_tc;
    param_2 = _e96;
    param_3 = 0i;
    let _e97 = wired_bl_sample_domain_u0028_u1_u003b_vf2_u003b_i1_u003b((&param_1), (&param_2), (&param_3));
    base = _e97;
    if override_type_6_ {
        if override_type_6_1 {
            let _e99 = base[3u];
            base[3u] = select(0f, 1f, (_e99 > 0f));
        } else {
            if override_type_6_2 {
                ...
                param_6 = _e106;
                let _e107 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_4), (&param_5), (&param_6));
                base[3u] = _e107;
            } else {
                if override_type_6_3 {
                    ...
                    param_9 = _e111;
                    let _e112 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_7), (&param_8), (&param_9));
                    base[3u] = _e112;
                }
            }
        }
    ...
    let _e124 = unnamed.lightColor;
    lightColorRadius = _e124;
    let _e125 = L_1;
    nL = normalize(_e125.xyz);
    let _e128 = V_1;
    nV = normalize(_e128.xyz);
    let _e131 = L_1;
    let _e133 = L_1;
    let _e137 = lightColorRadius[3u];
    intensFactor = (1f - (dot(_e131.xyz, _e133.xyz) * _e137));
    let _e140 = intensFactor;
    if (_e140 <= 0f) {
        discard;
    }
    let _e142 = lightColorRadius;
    let _e144 = intensFactor;
    intens = (_e142.xyz * _e144);
    let _e146 = wired_advanced_fog_amount_u0028_();
    fogAmount = _e146;
    let _e147 = wired_advanced_fog_enabled_u0028_();
    if !(_e147) {
        let _e150 = fog[3u];
        fogAmount = _e150;
    }
    let _e151 = fogAmount;
    let _e153 = base;
    let _e155 = (_e153.xyz * (1f - _e151));
    base[0u] = _e155.x;
    base[1u] = _e155.y;
    base[2u] = _e155.z;
    let _e162 = Np;
    let _e163 = nL;
    diffuse = dot(_e162, _e163);
    let _e165 = Np;
    let _e166 = nL;
    let _e167 = nV;
    specFactor = dot(_e165, normalize((_e166 + _e167)));
    if override_type_6_7 {
        let _e171 = diffuse;
        let _e172 = Np;
        let _e173 = nV;
        if ((_e171 * dot(_e172, _e173)) <= 0f) {
            discard;
        }
        let _e177 = diffuse;
        diffuse = abs(_e177);
        let _e179 = specFactor;
        specFactor = abs(_e179);
    } else {
        let _e181 = diffuse;
        diffuse = max(_e181, 0f);
        let _e183 = specFactor;
        specFactor = max(_e183, 0f);
    }
    let _e185 = specFactor;
    let _e189 = base;
    spec = ((vec4((pow(_e185, 10f) * 0.25f)) * _e189) * 0.8f);
    let _e192 = base;
    let _e193 = diffuse;
    let _e196 = spec;
    let _e198 = intens;
    out_color = (((_e192 * vec4(_e193)) + _e196) * vec4<f32>(_e198.x, _e198.y, _e198.z, 1f));
    return;
}
