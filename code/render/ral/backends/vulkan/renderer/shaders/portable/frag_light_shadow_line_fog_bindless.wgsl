...
fn main_1() {
    ...
    Np = _e123;
    param_5 = 0u;
    let _e124 = parallax_tc;
    param_6 = _e124;
    param_7 = 0i;
    let _e125 = wired_bl_sample_domain_u0028_u1_u003b_vf2_u003b_i1_u003b((&param_5), (&param_6), (&param_7));
    base = _e125;
    if override_type_8_2 {
        if override_type_8_3 {
            let _e127 = base[3u];
            base[3u] = select(0f, 1f, (_e127 > 0f));
        } else {
            if override_type_8_4 {
                ...
                param_10 = _e134;
                let _e135 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_8), (&param_9), (&param_10));
                base[3u] = _e135;
            } else {
                if override_type_8_5 {
                    ...
                    param_13 = _e139;
                    let _e140 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_11), (&param_12), (&param_13));
                    base[3u] = _e140;
                }
            }
        }
        ...
        if override_type_8_6 {
            let _e143 = base[3u];
            if (_e143 == alpha_test_value) {
                discard;
            }
        } else {
            if override_type_8_7 {
                let _e146 = base[3u];
                if (_e146 >= alpha_test_value) {
                    discard;
                }
            } else {
                if override_type_8_8 {
                    let _e149 = base[3u];
                    if (_e149 < alpha_test_value) {
                        discard;
                    }
                }
            }
    ...
    let _e152 = unnamed.lightColor;
    lightColorRadius = _e152;
    let _e153 = L_1;
    let _e157 = unnamed.lightVector;
    let _e162 = unnamed.lightVector[3u];
    scale_1 = clamp((dot(-(_e153.xyz), _e157.xyz) * _e162), 0f, 1f);
    let _e166 = unnamed.lightVector;
    let _e167 = scale_1;
    let _e169 = L_1;
    LL = ((_e166 * _e167) + _e169);
    let _e171 = LL;
    nL = normalize(_e171.xyz);
    let _e174 = V_1;
    nV = normalize(_e174.xyz);
    let _e177 = LL;
    let _e179 = LL;
    let _e183 = lightColorRadius[3u];
    intensFactor = (1f - (dot(_e177.xyz, _e179.xyz) * _e183));
    let _e186 = intensFactor;
    if (_e186 <= 0f) {
        discard;
    }
    let _e188 = lightColorRadius;
    let _e190 = intensFactor;
    intens = (_e188.xyz * _e190);
    dbgCascade = 0i;
    let _e192 = shadowData_1;
    param_14 = _e192.xyz;
    let _e195 = shadowData_1[3u];
    param_15 = _e195;
    let _e196 = sampleShadow_u0028_vf3_u003b_f1_u003b_i1_u003b((&param_14), (&param_15), (&param_16));
    let _e197 = param_16;
    dbgCascade = _e197;
    let _e198 = intens;
    intens = (_e198 * _e196);
    let _e200 = wired_advanced_fog_amount_u0028_();
    fogAmount = _e200;
    ...
    let _e205 = fogAmount;
    let _e207 = base;
    let _e209 = (_e207.xyz * (1f - _e205));
    base[0u] = _e209.x;
    base[1u] = _e209.y;
    base[2u] = _e209.z;
    let _e216 = Np;
    let _e217 = nL;
    diffuse = dot(_e216, _e217);
    let _e219 = Np;
    let _e220 = nL;
    let _e221 = nV;
    specFactor = dot(_e219, normalize((_e220 + _e221)));
    if override_type_8_9 {
        let _e225 = diffuse;
        let _e226 = Np;
        let _e227 = nV;
        if ((_e225 * dot(_e226, _e227)) <= 0f) {
            discard;
        }
        let _e231 = diffuse;
        diffuse = abs(_e231);
        let _e233 = specFactor;
        specFactor = abs(_e233);
    } else {
        let _e235 = diffuse;
        diffuse = max(_e235, 0f);
        let _e237 = specFactor;
        specFactor = max(_e237, 0f);
    }
    let _e239 = specFactor;
    let _e243 = base;
    spec = ((vec4((pow(_e239, 10f) * 0.25f)) * _e243) * 0.8f);
    let _e246 = base;
    let _e247 = diffuse;
    let _e250 = spec;
    let _e252 = intens;
    out_color = (((_e246 * vec4(_e247)) + _e250) * vec4<f32>(_e252.x, _e252.y, _e252.z, 1f));
    let _e260 = unnamed.eyePos[3u];
    if (_e260 > 0.5f) {
        let _e262 = dbgCascade;
        if (_e262 == 0i) {
            local_1 = vec3<f32>(1f, 0.3f, 0.3f);
        } else {
            let _e264 = dbgCascade;
            if (_e264 == 1i) {
                local_2 = vec3<f32>(0.3f, 1f, 0.3f);
            } else {
                let _e266 = dbgCascade;
                local_2 = select(vec3<f32>(1f, 1f, 0.3f), vec3<f32>(0.3f, 0.3f, 1f), vec3((_e266 == 2i)));
            }
            let _e270 = local_2;
            local_1 = _e270;
        }
        ...
        let _e272 = tint;
        let _e273 = out_color;
        let _e275 = (_e273.xyz * _e272);
        out_color[0u] = _e275.x;
        out_color[1u] = _e275.y;
        out_color[2u] = _e275.z;
    }
    return;
}
