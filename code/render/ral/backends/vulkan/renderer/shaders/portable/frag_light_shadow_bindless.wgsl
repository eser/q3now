...
fn main_1() {
    ...
    if override_type_8_2 {
        if override_type_8_3 {
            ...
            if override_type_8_4 {
                ...
                param_10 = _e115;
                let _e116 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_8), (&param_9), (&param_10));
                base[3u] = _e116;
            } else {
                if override_type_8_5 {
                    ...
                    param_13 = _e120;
                    let _e121 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_11), (&param_12), (&param_13));
                    base[3u] = _e121;
                }
            }
        }
    ...
    let _e133 = unnamed.lightColor;
    lightColorRadius = _e133;
    let _e134 = L_1;
    nL = normalize(_e134.xyz);
    let _e137 = V_1;
    nV = normalize(_e137.xyz);
    let _e140 = L_1;
    let _e142 = L_1;
    let _e146 = lightColorRadius[3u];
    intensFactor = (1f - (dot(_e140.xyz, _e142.xyz) * _e146));
    let _e149 = intensFactor;
    if (_e149 <= 0f) {
        discard;
    }
    let _e151 = lightColorRadius;
    let _e153 = intensFactor;
    intens = (_e151.xyz * _e153);
    dbgCascade = 0i;
    let _e155 = shadowData_1;
    param_14 = _e155.xyz;
    let _e158 = shadowData_1[3u];
    param_15 = _e158;
    let _e159 = sampleShadow_u0028_vf3_u003b_f1_u003b_i1_u003b((&param_14), (&param_15), (&param_16));
    let _e160 = param_16;
    dbgCascade = _e160;
    let _e161 = intens;
    intens = (_e161 * _e159);
    let _e163 = wired_advanced_fog_amount_u0028_();
    fogAmount = _e163;
    let _e164 = fogAmount;
    let _e166 = base;
    let _e168 = (_e166.xyz * (1f - _e164));
    base[0u] = _e168.x;
    base[1u] = _e168.y;
    base[2u] = _e168.z;
    let _e175 = Np;
    let _e176 = nL;
    diffuse = dot(_e175, _e176);
    let _e178 = Np;
    let _e179 = nL;
    let _e180 = nV;
    specFactor = dot(_e178, normalize((_e179 + _e180)));
    if override_type_8_9 {
        let _e184 = diffuse;
        let _e185 = Np;
        let _e186 = nV;
        if ((_e184 * dot(_e185, _e186)) <= 0f) {
            discard;
        }
        let _e190 = diffuse;
        diffuse = abs(_e190);
        let _e192 = specFactor;
        specFactor = abs(_e192);
    } else {
        let _e194 = diffuse;
        diffuse = max(_e194, 0f);
        let _e196 = specFactor;
        specFactor = max(_e196, 0f);
    }
    let _e198 = specFactor;
    let _e202 = base;
    spec = ((vec4((pow(_e198, 10f) * 0.25f)) * _e202) * 0.8f);
    let _e205 = base;
    let _e206 = diffuse;
    let _e209 = spec;
    let _e211 = intens;
    out_color = (((_e205 * vec4(_e206)) + _e209) * vec4<f32>(_e211.x, _e211.y, _e211.z, 1f));
    let _e219 = unnamed.eyePos[3u];
    if (_e219 > 0.5f) {
        let _e221 = dbgCascade;
        if (_e221 == 0i) {
            local_1 = vec3<f32>(1f, 0.3f, 0.3f);
        } else {
            let _e223 = dbgCascade;
            if (_e223 == 1i) {
                local_2 = vec3<f32>(0.3f, 1f, 0.3f);
            } else {
                let _e225 = dbgCascade;
                local_2 = select(vec3<f32>(1f, 1f, 0.3f), vec3<f32>(0.3f, 0.3f, 1f), vec3((_e225 == 2i)));
            }
            let _e229 = local_2;
            local_1 = _e229;
        }
        ...
        out_color[0u] = _e234.x;
        out_color[1u] = _e234.y;
        out_color[2u] = _e234.z;
    }
    return;
}
