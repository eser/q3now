...
fn main_1() {
    ...
    if _e73 {
        let _e74 = wired_advanced_fog_amount_u0028_();
        fogAmount = _e74;
        let _e75 = base;
        let _e78 = unnamed.advancedFogColorDensity;
        let _e80 = fogAmount;
        let _e82 = mix(_e75.xyz, _e78.xyz, vec3(_e80));
        base[0u] = _e82.x;
        base[1u] = _e82.y;
        base[2u] = _e82.z;
    }
    if override_type_3_3 {
        ...
        if override_type_3_4 {
            let _e92 = base;
            let _e94 = base;
            if (dot(_e92.xyz, _e94.xyz) == 0f) {
                discard;
            }
        }
    }
    let _e98 = gl_FragCoord_1;
    let _e103 = unnamed.packed_indices[1i][0u];
    let _e109 = unnamed.packed_indices[1i][0u];
    let _e114 = textureDimensions(wired_bindless_images[(_e103 & 4095u)], 0i);
    screenUV = (_e98.xy / vec2<f32>(vec2<i32>(_e114)));
    let _e121 = unnamed.packed_indices[1i][0u];
    let _e127 = unnamed.packed_indices[1i][0u];
    let _e132 = screenUV;
    let _e133 = textureSample(wired_bindless_images[(_e121 & 4095u)], wired_bindless_samplers[((_e127 >> bitcast<u32>(12i)) & 255u)], _e132);
    sceneDepth = _e133.x;
    let _e136 = gl_FragCoord_1[2u];
    fragDepth = _e136;
    let _e137 = fragDepth;
    let _e138 = sceneDepth;
    depthDiff = (_e137 - _e138);
    let _e141 = depthDiff;
    fadeFactor = smoothstep(0f, (depth_fade_scale * 0.0005f), _e141);
    let _e143 = fadeFactor;
    let _e145 = base[3u];
    base[3u] = (_e145 * _e143);
    let _e148 = fadeFactor;
    let _e149 = base;
