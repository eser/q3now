...
const TARGETS = ['msl', 'glsl460', 'wgsl'];
...
function parseTargets(value) {
	...
	if (!targets.length || targets.some((target) => !TARGETS.includes(target))
		...
		throw new Error('targets must be a unique canonical subset of msl,glsl460,wgsl');
	return targets;
}
...
function validatePortableManifest(text, source, requireFreshSource) {
	let manifest;
	...
	if (manifest.schemaVersion !== 1 || !Number.isSafeInteger(manifest.generation) || manifest.generation <= 0
			...
			|| (requireFreshSource && manifest.entries.length !== source.length))
	...
	for (let i = 0; i < manifest.entries.length; ++i) {
		...
		if (entry.ordinal !== i || entry.symbol !== row.symbol || entry.stage !== row.stage
				|| (requireFreshSource && (!digestHexExact(entry.sourceDigest, row.inputDigest)
...
function glBindingNamespace(bindingClass) {
	if (bindingClass === 'RAL_SHADER_BIND_UNIFORM_BUFFER') return 'uniformBuffer';
	if (bindingClass === 'RAL_SHADER_BIND_STORAGE_BUFFER_READ'
			|| bindingClass === 'RAL_SHADER_BIND_STORAGE_BUFFER_READ_WRITE') return 'storageBuffer';
	if (bindingClass === 'RAL_SHADER_BIND_SAMPLED_TEXTURE') return 'sampledTexture';
	if (bindingClass === 'RAL_SHADER_BIND_STORAGE_TEXTURE_READ'
			|| bindingClass === 'RAL_SHADER_BIND_STORAGE_TEXTURE_WRITE') return 'storageTexture';
	if (bindingClass === 'RAL_SHADER_BIND_FILTERING_SAMPLER'
			|| bindingClass === 'RAL_SHADER_BIND_NONFILTERING_SAMPLER'
			|| bindingClass === 'RAL_SHADER_BIND_COMPARISON_SAMPLER') return 'sampler';
	throw new Error(`unsupported GLSL 4.60 binding class: ${bindingClass}`);
}

export function lowerGlsl460(bytes, portableEntry) {
	let source = Buffer.from(bytes).toString('utf8');
	const reflection = portableEntry?.reflection;
	if (!reflection || !Array.isArray(reflection.bindings) || !source.startsWith('#version 460\n'))
		throw new Error('GLSL 4.60 lowering requires exact portable reflection');
	const counters = new Map(); const bindings = new Map();
	const inline = reflection.inlineData?.byteSize > 0 ? reflection.inlineData : null;
	for (const binding of reflection.bindings) {
		const namespace = glBindingNamespace(binding.bindingClass);
		let index = counters.get(namespace) ?? 0;
		if (namespace === 'uniformBuffer' && inline && index === inline.webgpuUniformBinding) index++;
		bindings.set(`${binding.set}:${binding.binding}`, { ...binding, glBinding: index });
		counters.set(namespace, index + 1);
	}
	source = source.replace('#extension GL_EXT_nonuniform_qualifier : require\n', '');
	source = source.replace(/layout\(set = (\d+), binding = (\d+)([^)]*)\)([^\n]*)/g,
		(_match, set, binding, suffix, declaration) => {
			const mapped = bindings.get(`${set}:${binding}`);
			if (!mapped) throw new Error(`GLSL binding missing from portable reflection: ${set}:${binding}`);
			let lowered = declaration;
			if (mapped.arrayCount > 1) {
				const bounded = Math.min(mapped.arrayCount, 64);
				lowered = lowered.includes('[]')
					? lowered.replace('[]', `[${bounded}]`)
					: lowered.replace(/\[\d+\]/, `[${bounded}]`);
				if (!lowered.includes(`[${bounded}]`))
					throw new Error(`GLSL descriptor array lowering failed: ${set}:${binding}`);
			}
			return `layout(binding = ${mapped.glBinding}${suffix})${lowered}`;
		});
	if (inline) source = source.replace(/layout\(push_constant(?:,\s*std430)?\)/g,
		`layout(std140, binding = ${inline.webgpuUniformBinding})`);
	for (;;) {
		const lowered = source.replace(/nonuniformEXT\(([^()]+)\)/g, '($1)');
		if (lowered === source) break;
		source = lowered;
	}
	...
	if (/layout\([^)]*set\s*=|push_constant|nonuniformEXT|GL_EXT_nonuniform_qualifier/.test(source))
		throw new Error('GLSL 4.60 lowering left Vulkan-only syntax');
	return Buffer.from(source);
}

function runTranslator(translator, item, portableEntry, workDir, targets, requireWgsl) {
	...
	for (const target of targets) {
		...
		if (target === 'glsl460') bytes = lowerGlsl460(bytes, portableEntry);
		if (target === 'wgsl') {
			const source = bytes.toString('utf8').replace(/[ \t]+$/gm, '');
...
export function runCorpus(options) {
	...
	// A write-mode refresh intentionally permits the previous canonical
	// portable manifest as a bootstrap input: translate the new SPIR-V, refresh
	// reflection/portable metadata, then rerun this writer to seal its digest.
	// --check remains strict and rejects every stale source identity.
