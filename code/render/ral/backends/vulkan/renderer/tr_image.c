...
// tr_image.c
#include "tr_local.h"
#include "../../../../frontend/r_log.h"  // rilog-channel-mechanism — renderer.assets
#ifdef USE_VULKAN
...
static void upload_vk_image( image_t *image, byte *pic ) {
	...
	if ( !( image->flags & IMGFLAG_STREAM_ASSET_CHUNK )
			|| upload_data.mip_levels != 1
			|| !vk_ral_queue_asset_chunk_image( image, upload_data.buffer,
				w, h, (uint64_t)upload_data.buffer_size ) ) {
		vk_upload_image_data( image, 0, 0, w, h, upload_data.mip_levels,
			upload_data.buffer, upload_data.buffer_size, qfalse, 0 );
