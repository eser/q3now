...
typedef enum
{
	...
	/* The caller supplied a complete decoded page whose upload may be admitted
	   later by the bounded RAL asset-chunk scheduler. The image is published as
	   the resident default fallback until its exact upload ticket completes at
	   the render-thread safe boundary. Renderer-internal; currently used by BSP
	   lightmap pages, and deliberately backend-neutral for the WebGPU path. */
	IMGFLAG_STREAM_ASSET_CHUNK = 0x20000,
} imgFlags_t;
