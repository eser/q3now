...
static void R_SetupProjectionZ( viewParms_t *dest )
{
	...
	// (Vulkan/OpenGL 4.6/WebGPU unclippedDepth) — handled entirely in the raster state,
	// nothing to do here. On a backend WITHOUT native depth-clamp (capability-false,
	// capability-false) the pipeline bit stays off, so instead pull the near plane very
	...
	if ( r_depthClamp && r_depthClamp->integer && !vk_depth_clamp_supported() ) {
		...
		// enough to preserve usable depth precision.
