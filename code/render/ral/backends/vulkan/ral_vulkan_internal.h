...
// the helpers.
//
// IMPORTANT: this backend is deliberately *independent* of code/render/ral/backends/vulkan/renderer/.
...
#include "../../core/ral.h"                       // q_shared.h + the public RAL surface
#include "../../core/ral_host.h"                  // backend-neutral host contract helpers
...
struct ralBuffer_s {
	...
	void               *immediateMapped; // backend-owned persistent mapping for immediate writes
...
static inline qboolean ralVk_BackendHasGpuExcludedBufferMaps(
...
static inline qboolean ralVk_BindGroupBuffersGpuUseAllowed( const ralBindGroup_t *group ) {
	...
	if ( !group ) return qfalse;
	if ( group->backend
			&& !ralVk_BackendHasGpuExcludedBufferMaps( group->backend ) )
		return qtrue;
...
static inline qboolean ralVk_CommandBoundBuffersGpuUseAllowed( const ralCommandBuffer_t *cb ) {
	uint32_t i;
	if ( !cb || !cb->backend ) return qfalse;
	if ( !ralVk_BackendHasGpuExcludedBufferMaps( cb->backend ) ) return qtrue;
...
struct ralBackend_s {
	...
	uint64_t          gpuExcludedBufferMapCount; // public maps requiring bound-resource revalidation
...
static inline qboolean ralVk_BackendHasGpuExcludedBufferMaps(
		const ralBackend_t *backend ) {
	return backend && backend->gpuExcludedBufferMapCount > 0u;
}
