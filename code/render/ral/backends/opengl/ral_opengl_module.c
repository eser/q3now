...
static qboolean ReceiptValid( const ralOpenGlModuleFrameReceipt_t *receipt ) {
	return receipt && receipt->schemaVersion == RAL_OPENGL_MODULE_SCHEMA_VERSION
		...
		&& RalOpenGl_ProductFrameReceiptExact( &receipt->product,
			&receipt->product )
		...
		&& receipt->product.unresolvedCount == 0u
		&& receipt->product.fallbackCount == 0u
...
static void LogFailure( const char *reason ) {
	if ( !s_module.exports.initFailed && s_module.imports.LogCh
		...
		s_module.imports.LogCh( s_module.logChannel, SEV_WARN,
			"Wired native OpenGL RAL: lifecycle failure (%s)\n", reason );
...
qboolean RB_ScheduleScreenshot( int typeMask, const char *fileName,
	...
	if ( typeMask != SCREENSHOT_PNG || !fileName || !fileName[0]
			|| s_module.screenshotPending || !s_module.product
			|| strlen( fileName ) >= sizeof( s_module.screenshotName ) ) return qfalse;
...
static qboolean CompleteScreenshot( void ) {
	...
	bytes = (uint64_t)s_module.config.vidWidth
		* (uint64_t)s_module.config.vidHeight * 3u;
	if ( s_module.config.vidWidth <= 0 || s_module.config.vidHeight <= 0
			|| bytes > UINT32_MAX ) return qfalse;
	pixels = (byte *)malloc( (size_t)bytes );
	if ( !pixels ) return qfalse;
	if ( !RalOpenGl_ProductReadbackRgb( s_module.product, pixels,
			(uint32_t)bytes )
			|| !R_SavePNG( s_module.screenshotName, pixels,
				s_module.config.vidWidth, s_module.config.vidHeight ) ) {
		free( pixels ); return qfalse;
	}
	free( pixels );
...
static ralOpenGlProc_t ResolveGl( const char *name, void *userData ) {
	...
	if ( !module || !module->imports.GL_GetProcAddress ) return NULL;
...
static qhandle_t RegisterImageSource( renderAssetKind_t kind, const char *name,
		const char *imageName, qboolean clamp ) {
	...
	if ( name && RenderMaterialScript_Lookup( &s_module.materialScripts, name,
			&scripted ) ) {
	...
	if ( name && ( name[0] == '*' || !strcmp( name, "noshader" ) ) )
		return RenderSubmission_RegisterMaterialImage( &s_module.frontend, kind,
			name, qtrue, white, 1u, 1u );
	if ( imageName && RenderImage_DecodeRgba8( imageName, &pixels, &width,
			&height, resolved ) ) {
...
static qhandle_t RegisterImage( renderAssetKind_t kind, const char *name,
		qboolean clamp ) {
...
static qhandle_t RegisterModel( const char *name ) {
	...
	if ( !s_module.imports.FS_ReadFile || !s_module.imports.FS_FreeFile ) return 0;
	byteCount = s_module.imports.FS_ReadFile( name, &bytes );
	if ( byteCount <= 0 || !bytes ) { if ( bytes ) s_module.imports.FS_FreeFile( bytes ); return 0; }
	handle = RenderSubmission_RegisterModelData( &s_module.frontend, name, bytes,
		(uint32_t)byteCount );
	...
	for ( uint32_t i = 0u; i < model.batchCount; ++i ) {
		qhandle_t material = RegisterImage( RENDER_ASSET_MATERIAL,
...
static qhandle_t RegisterLightMap( const char *name, int lightmap ) {
	(void)lightmap; return RegisterImage( RENDER_ASSET_LIGHTMAP, name, qtrue );
}
...
static qboolean PrepareWorldMaterials( const mapFile_t *bsp ) {
	...
	uint32_t side = 0u;
	uint64_t pixels;
	if ( !bsp || bsp->numSurfaces < 0 || bsp->numShaders < 0
			|| bsp->numLightmapPages < 0
	...
	for ( int i = 0; i < bsp->numSurfaces; ++i ) {
		...
		if ( shader->surfaceFlags & ( SURF_NODRAW | SURF_SKIP ) ) continue;
		if ( !RenderSubmission_MaterialHandle( &s_module.frontend, shader->shader )
			...
			if ( s_module.imports.LogCh && s_module.logChannel >= 0 )
				s_module.imports.LogCh( s_module.logChannel, SEV_WARN,
					...
					i, surface->shaderNum, shader->shader );
			return qfalse;
		}
	}
	...
	for ( int i = 0; i < bsp->numLightmapPages; ++i ) {
		...
		const byte *rgb = bsp->lightmapData
			+ (size_t)i * (size_t)bsp->lightmapPageSize;
...
static qboolean InitializeOwners( void ) {
	...
	if ( !coreGeneration || !s_module.imports.GLimp_InitOpenGL46
			|| !s_module.imports.GLimp_Shutdown
			|| !s_module.imports.GLimp_EndFrame
			|| !s_module.imports.GL_GetProcAddress ) return qfalse;
	...
	if ( !RalOpenGl_CoreCreate( &info, &s_module.core, &s_module.coreReceipt ) ) {
		LogFailure( "core-create" ); goto fail;
	}
	if ( !RalOpenGl_ProductCreate( s_module.core, &s_module.coreReceipt,
			s_module.moduleGeneration, &s_module.product ) ) {
		LogFailure( "product-create" ); goto fail;
	}
	if ( !RalOpenGl_ProductSetOutputExtent( s_module.product,
			(uint32_t)s_module.config.vidWidth,
			(uint32_t)s_module.config.vidHeight ) ) {
		LogFailure( "product-output-extent" ); goto fail;
	}
	(void)snprintf( s_module.config.renderer_string,
		sizeof( s_module.config.renderer_string ), "Wired native OpenGL 4.6 RAL" );
	...
	(void)snprintf( s_module.config.version_string,
		sizeof( s_module.config.version_string ), "OpenGL %u.%u Core",
	...
	return qtrue;
fail:
	RalOpenGl_ProductDestroy( s_module.product ); s_module.product = NULL;
	RalOpenGl_CoreDestroy( s_module.core ); s_module.core = NULL;
...
static void BeginRegistration( glconfig_t *config ) {
	if ( !config || !s_module.loaded || s_module.frameOpen ) {
		LogFailure( "begin-registration-state" ); return;
	}
	if ( !s_module.core && !InitializeOwners() ) {
		LogFailure( "owner-initialization" ); return;
	}
	if ( !s_module.frontend.initialized
			&& !RenderSubmission_Init( &s_module.frontend,
				s_module.moduleGeneration ) ) {
		LogFailure( "frontend-initialization" ); return;
	}
	if ( !s_module.materialScripts.ready
			&& !RenderMaterialScript_Load( &s_module.materialScripts, &ri ) ) {
		LogFailure( "material-script-catalog" ); return;
	}
	if ( !s_module.defaultMaterial )
		s_module.defaultMaterial = RegisterImage( RENDER_ASSET_MATERIAL,
			"*white", qtrue );
	if ( !s_module.defaultMaterial ) { LogFailure( "default-material" ); return; }
...
static void EndRegistration( void ) {
	if ( !s_module.registered || s_module.frameOpen ) LogFailure( "end-registration-state" );
}
...
static void BeginFrame( stereoFrame_t stereo ) {
	...
	if ( !generation || !s_module.registered || s_module.frameOpen
		...
		LogFailure( "begin-frame-state" ); return;
	}
...
static void EndFrame( int *frontEndMsec, int *backEndMsec ) {
	...
	if ( !s_module.frameOpen ) { LogFailure( "end-frame-state" ); return; }
	...
	if ( !RenderSubmission_EndFrame( &s_module.frontend,
			receipt.frameGeneration, &receipt.frontend ) ) {
		s_module.frameOpen = qfalse;
		s_module.currentFrameGeneration = 0u;
		LogFailure( "frontend-end-frame" ); return;
	}
	memset( &planProbe, 0, sizeof( planProbe ) );
	if ( !RalOpenGl_FrontendPlanBuild( &s_module.coreReceipt,
			&s_module.frontend, &receipt.frontend,
			receipt.frameGeneration, &planProbe ) ) {
		s_module.frameOpen = qfalse;
		s_module.currentFrameGeneration = 0u;
		LogFailure( "frontend-plan" ); return;
	}
	if ( !RalOpenGl_ProductRender( s_module.product, &s_module.frontend,
			&receipt.frontend, receipt.frameGeneration, &receipt.product ) ) {
		s_module.frameOpen = qfalse;
		s_module.currentFrameGeneration = 0u;
		LogFailure( "product-render" ); return;
	}
	if ( !CompleteScreenshot() ) {
		s_module.frameOpen = qfalse;
		s_module.currentFrameGeneration = 0u;
		LogFailure( "screenshot-capture" ); return;
	}
	s_module.imports.GLimp_EndFrame();
	receipt.presentedFrames = ++s_module.presentedFrames;
	receipt.core = s_module.coreReceipt; receipt.ready = qtrue;
	s_module.frameOpen = qfalse;
	s_module.currentFrameGeneration = 0u;
	if ( !ReceiptValid( &receipt ) ) { LogFailure( "published-receipt" ); return; }
	...
	logContent = worldChanged || ( completeContent
		&& !s_module.loggedCompleteContentReceipt );
	if ( receipt.frontend.worldLoaded && receipt.frontend.sceneRendered
			&& logContent && s_module.imports.LogCh
			&& s_module.receiptLogChannel >= 0 ) {
		s_module.imports.LogCh( s_module.receiptLogChannel, SEV_INFO,
			"Wired native OpenGL RAL: content receipt map=%s asset=%016llx world=%016llx frame=%016llx surfaces=%u vertices=%u indices=%u assets=%u materials=%u resolvedMaterials=%u materialBytes=%u models=%u modelBytes=%u entities=%u temporalEntities=%u polygons=%u lights=%u ui=%u loweredWorld=%u worldBatches=%u loweredEntity=%u entityBatches=%u modelEntities=%u primitiveEntities=%u loweredPolygons=%u loweredLights=%u effectBatches=%u loweredUi=%u texturedUi=%u msdfUi=%u nativeDraws=%u unresolved=%u fallback=%u fatal=%u\n",
			s_module.loadedWorld ? s_module.loadedWorld->name : "<unknown>",
			(unsigned long long)receipt.frontend.assetDigest,
			(unsigned long long)receipt.frontend.worldDigest,
			(unsigned long long)receipt.frontend.frameDigest,
			receipt.frontend.worldSurfaceCount,
			receipt.frontend.worldVertexCount,
			receipt.frontend.worldIndexCount,
			receipt.frontend.registeredAssetCount,
			receipt.frontend.registeredMaterialCount,
			receipt.frontend.resolvedMaterialCount,
			receipt.frontend.materialBytes,
			receipt.frontend.registeredModelCount,
			receipt.frontend.modelBytes,
			receipt.frontend.entityCount,
			receipt.frontend.temporalEntityCount,
			receipt.frontend.polygonCount,
			receipt.frontend.lightCount,
			receipt.frontend.uiPrimitiveCount,
			receipt.product.plan.loweredWorldIndexCount,
			receipt.product.plan.loweredWorldBatchCount,
			receipt.product.plan.loweredEntityIndexCount,
			receipt.product.plan.loweredEntityBatchCount,
			receipt.product.plan.loweredModelEntityCount,
			receipt.product.plan.loweredPrimitiveEntityCount,
			receipt.product.plan.loweredPolygonCount,
			receipt.product.plan.loweredLightCount,
			receipt.product.plan.loweredEffectBatchCount,
			receipt.product.plan.loweredUiPrimitiveCount,
			receipt.product.plan.texturedUiPrimitiveCount,
			receipt.product.plan.msdfUiPrimitiveCount,
			receipt.product.native.nativeDrawCount,
			receipt.product.unresolvedCount,
			receipt.product.fallbackCount,
			receipt.product.fatalCount );
...
static void Shutdown( refShutdownCode_t code ) {
	...
	s_module.glInitialized = qfalse;
	if ( s_module.imports.LogCh && s_module.logChannel >= 0 )
		s_module.imports.LogCh( s_module.logChannel, SEV_INFO,
			"Wired native OpenGL RAL: shutdown complete code=%d\n", (int)code );
	memset( &s_module.published, 0, sizeof( s_module.published ) );
	s_module.loaded = qfalse; s_module.registered = qfalse;
}
...
static qboolean PrepareEntity( const refEntity_t *entity,
		refEntity_t *outSubmitted ) {
	if ( !s_module.frameOpen || !entity || !outSubmitted ) return qfalse;
	if ( (unsigned)entity->reType >= RT_MAX_REF_ENTITY_TYPE ) {
		if ( s_module.imports.Terminate )
			s_module.imports.Terminate( TERM_CLIENT_DROP,
				"OpenGL RAL AddEntity: bad reType %d", entity->reType );
		return qfalse;
	}
	...
	if ( isnan( entity->origin[0] ) || isnan( entity->origin[1] )
			|| isnan( entity->origin[2] ) ) {
		if ( !s_module.loggedInvalidEntityOrigin && s_module.imports.LogCh
				&& s_module.logChannel >= 0 ) {
	...
	*outSubmitted = *entity;
	if ( outSubmitted->reType == RT_MODEL && outSubmitted->hModel == 0
			&& outSubmitted->customShader == 0 )
...
static void AddEntity( const refEntity_t *entity, qboolean shaderTime ) {
	...
	(void)shaderTime;
	if ( !PrepareEntity( entity, &submitted ) ) return;
	if ( !RenderSubmission_AddEntity( &s_module.frontend, &submitted, NULL ) )
		LogFailure( "entity" );
}
static void AddEntityTemporal( const refEntity_t *entity,
		const refEntityMotion_t *motion ) {
	refEntity_t submitted;
	if ( !s_module.frameOpen || !entity || !RefEntityMotion_IsValid( motion ) )
		return;
	if ( !PrepareEntity( entity, &submitted ) ) return;
	if ( !RenderSubmission_AddEntity( &s_module.frontend, &submitted, motion ) )
		LogFailure( "temporal-entity" );
}
static void AddPoly( qhandle_t shader, int vertices, const polyVert_t *data,
		int count ) {
	if ( s_module.frameOpen && !RenderSubmission_AddPoly( &s_module.frontend,
			shader, vertices, data, count ) ) LogFailure( "poly" );
}
static void AddLight( const vec3_t origin, float intensity, float red,
		float green, float blue ) {
	if ( s_module.frameOpen && !RenderSubmission_AddLight( &s_module.frontend,
			origin, NULL, intensity, red, green, blue ) ) LogFailure( "light" );
}
static void AddLinearLight( const vec3_t start, const vec3_t end,
		float intensity, float red, float green, float blue ) {
	if ( s_module.frameOpen && !RenderSubmission_AddLight( &s_module.frontend,
			start, end, intensity, red, green, blue ) ) LogFailure( "linear-light" );
}
static void RenderScene( const refdef_t *view, int worldIndex ) {
	if ( s_module.frameOpen && !RenderSubmission_RenderScene( &s_module.frontend,
			view, worldIndex ) ) LogFailure( "scene" );
}
static void SetColor( const float *color ) {
	if ( !RenderSubmission_SetColor( &s_module.frontend, color ) ) LogFailure( "color" );
}
static void DrawPic( float x, float y, float width, float height,
		float s1, float t1, float s2, float t2, qhandle_t shader ) {
	if ( s_module.frameOpen && !RenderSubmission_AddUiQuad( &s_module.frontend,
			x, y, width, height, s1, t1, s2, t2, 0.0f, shader ) ) LogFailure( "ui" );
}
static void DrawRotatedPic( float x, float y, float width, float height,
		float s1, float t1, float s2, float t2, float angle, qhandle_t shader ) {
	if ( s_module.frameOpen && !RenderSubmission_AddUiQuad( &s_module.frontend,
			x, y, width, height, s1, t1, s2, t2, angle, shader ) ) LogFailure( "ui-rotated" );
}
static void DrawLine( float x1, float y1, float x2, float y2, float width,
		qhandle_t shader ) {
	if ( s_module.frameOpen && !RenderSubmission_AddUiLine( &s_module.frontend,
			x1, y1, x2, y2, width, shader ) ) LogFailure( "ui-line" );
}
static void DrawBackdrop( float x, float y, float width, float height,
		float time, float mouseX, float mouseY, float transition ) {
	DrawPic( x, y, width, height, time, mouseX, mouseY, transition,
		s_module.defaultMaterial );
}

static void NoopVoid( void ) {}
static void NoopBool( qboolean value ) { (void)value; }
static void NoopHandle( qhandle_t value ) { (void)value; }
static void NoopBytes( const byte *value ) { (void)value; }
static int NoLightForPoint( vec3_t point, vec3_t ambient, vec3_t directed,
		vec3_t direction ) { (void)point; if ( ambient ) memset( ambient,0,sizeof(vec3_t) ); if ( directed ) memset( directed,0,sizeof(vec3_t) ); if ( direction ) memset( direction,0,sizeof(vec3_t) ); return 0; }
static void NoRibbon( const ribbonDesc_t *v ) { (void)v; }
static void NoRail( const railRibbonDesc_t *v ) { (void)v; }
static void NoBeam( const beamDesc_t *v ) { (void)v; }
static void NoSprite( const spriteDesc_t *v ) { (void)v; }
static void NoEmitter( const emitterDesc_t *v ) { (void)v; }
static void NoDecal( const decalDesc_t *v ) { (void)v; }
static void NoParticle( particleClassHandle_t h, const particleClass_t *v ) { (void)h;(void)v; }
static void NoAtmosphere( const atmosphericDesc_t *v ) { (void)v; }
static void NoHeightgrid( const float *v, int c ) { (void)v;(void)c; }
static void NoLens( const lensSourceDesc_t *v ) { (void)v; }
static qboolean NoLensVisibility( int id, float *v ) { (void)id;if(v)*v=0;return qfalse; }
static void NoClip( const float *v ) { (void)v; }
static void NoOutline( float w,const float*c,float g,const float*gc ){(void)w;(void)c;(void)g;(void)gc;}
static void NoShadow( float x,float y,const float*c ){(void)x;(void)y;(void)c;}
static void NoRaw( int x,int y,int w,int h,int c,int r,byte*d,int n,qboolean dirty ){(void)x;(void)y;(void)w;(void)h;(void)c;(void)r;(void)d;(void)n;(void)dirty;}
static void NoUpload( int w,int h,int c,int r,byte*d,int n,qboolean dirty ){(void)w;(void)h;(void)c;(void)r;(void)d;(void)n;(void)dirty;}
...
static int NoTag( orientation_t*t,qhandle_t m,int s,int e,float f,const char*n ){(void)t;(void)m;(void)s;(void)e;(void)f;(void)n;return 0;}
static void NoBounds( qhandle_t m,vec3_t mins,vec3_t maxs ){(void)m;if(mins)memset(mins,0,sizeof(vec3_t));if(maxs)memset(maxs,0,sizeof(vec3_t));}
static void NoFont( const char*n,int s,fontInfo_t*f ){(void)n;(void)s;if(f)memset(f,0,sizeof(*f));}
static void NoRemap( const char*a,const char*b,const char*c ){(void)a;(void)b;(void)c;}
static qboolean NoToken( char*b,int s ){if(b&&s>0)b[0]='\0';return qfalse;}
static qboolean NoPvs( const vec3_t a,const vec3_t b ){(void)a;(void)b;return qfalse;}
static void NoVideo( int h,int w,byte*c,byte*e,qboolean m ){(void)h;(void)w;(void)c;(void)e;(void)m;}
static qboolean CanMinimize( void ) { return qtrue; }
static const glconfig_t *GetConfig( void ) { return s_module.core ? &s_module.config : NULL; }
static qboolean GetMemoryBudget( uint64_t*d,uint64_t*db,uint64_t*h,uint64_t*hb,int*p ){if(d)*d=0;if(db)*db=0;if(h)*h=0;if(hb)*hb=0;if(p)*p=0;return qfalse;}
static int NoMdlAnimations( qhandle_t m,mdlAnimRange_t*a,int c ){(void)m;(void)a;(void)c;return 0;}
static void NoLightstyle( int s,const char*p ){(void)s;(void)p;}
static qboolean NoGpuProfile( refGpuProfileSample_t*s ){if(s)memset(s,0,sizeof(*s));return qfalse;}
static void PresentationChanged( const refPresentationChange_t *change ) {
	if ( !change || !s_module.product ) return;
	if ( change->logicalWidth > 0 && change->logicalHeight > 0
			&& !RalOpenGl_ProductSetOutputExtent( s_module.product,
				(uint32_t)change->logicalWidth, (uint32_t)change->logicalHeight ) )
		LogFailure( "presentation-extent" );
}

static void FillExports( refexport_t *e ) {
	memset( e, 0, sizeof( *e ) );
	e->Shutdown=Shutdown;e->BeginRegistration=BeginRegistration;e->RegisterModel=RegisterModel;
	e->RegisterSkin=RegisterSkin;e->RegisterShader=RegisterShader;e->RegisterShaderNoMip=RegisterShaderNoMip;
	e->RegisterShaderLightMap=RegisterLightMap;e->RegisterMSDFShader=RegisterMsdf;e->RegisterPrimitiveShader=RegisterPrimitive;e->PinShaderImages=NoopHandle;
	e->LoadWorld=LoadWorld;e->SetWorldVisData=NoopBytes;e->EndRegistration=EndRegistration;e->ClearScene=ClearScene;
	e->AddRefEntityToScene=AddEntity;e->AddPolyToScene=AddPoly;e->LightForPoint=NoLightForPoint;e->AddLightToScene=AddLight;e->AddAdditiveLightToScene=AddLight;e->AddLinearLightToScene=AddLinearLight;
	e->AddRibbonToScene=NoRibbon;e->AddRailRibbonToScene=NoRail;e->AddBeamToScene=NoBeam;e->AddSpriteToScene=NoSprite;e->EmitParticles=NoEmitter;e->AddDecalToScene=NoDecal;e->RegisterParticleClass=NoParticle;e->SetAtmosphere=NoAtmosphere;e->SetAtmosphereHeightgrid=NoHeightgrid;e->AddLensSourceToScene=NoLens;e->GetLensVisibility=NoLensVisibility;
	e->RenderScene=RenderScene;e->SetColor=SetColor;e->SetMSDFOutline=NoOutline;e->SetMSDFShadow=NoShadow;e->SetClipRegion=NoClip;e->DrawStretchPic=DrawPic;e->DrawMenuBackdrop=DrawBackdrop;e->DrawStretchPicOverlay=DrawPic;e->DrawRotatedPic=DrawRotatedPic;e->DrawLine=DrawLine;e->DrawStretchRaw=NoRaw;e->UploadCinematic=NoUpload;
	e->BeginFrame=BeginFrame;e->EndFrame=EndFrame;e->MarkFragments=NoFragments;e->LerpTag=NoTag;e->ModelBounds=NoBounds;e->RegisterFont=NoFont;e->RemapShader=NoRemap;e->GetEntityToken=NoToken;e->inPVS=NoPvs;e->TakeVideoFrame=NoVideo;e->ThrottleBackend=NoopVoid;e->FinishBloom=NoopVoid;e->SetColorMappings=NoopVoid;e->CanMinimize=CanMinimize;e->GetConfig=GetConfig;e->GetMemoryBudget=GetMemoryBudget;e->VertexLighting=NoopBool;e->SyncRender=NoopVoid;e->GetMDLAnimations=NoMdlAnimations;e->SetLightstylePattern=NoLightstyle;e->GetGpuProfileSample=NoGpuProfile;e->AddRefEntityToSceneTemporal=AddEntityTemporal;e->PresentationChanged=PresentationChanged;
}
...
Q_EXPORT refexport_t *QDECL GetRefAPI( int apiVersion, refimport_t *imports ) {
	if ( !imports || apiVersion != REF_API_VERSION || s_module.loaded
			|| s_moduleCounter >= UINT64_MAX - 1u || !imports->GLimp_InitOpenGL46
			|| !imports->GLimp_Shutdown || !imports->GLimp_EndFrame
			|| !imports->GL_GetProcAddress ) return NULL;
	memset( &s_module, 0, sizeof( s_module ) );
	s_module.imports = *imports; s_module.moduleGeneration = ++s_moduleCounter;
	ri = *imports;
	s_module.nextGeneration = s_module.moduleGeneration;
	s_module.logChannel = -1; s_module.receiptLogChannel = -1;
