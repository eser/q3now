...
// Mission objectives (game → cgame). Single string packing the in-memory
// objective list as "key,required,completed" triples joined by ';', e.g.
// "objective/find_key,1,0;objective/reach_exit,1,1". cgame resolves each key to
// localized text and pushes the list to the objectives HUD element. In-memory /
// per-session only — reset on map load, never serialized.
#define CS_OBJECTIVES		(CS_LIGHTSTYLES + CS_MAX_LIGHTSTYLES)
#if (CS_OBJECTIVES) >= MAX_CONFIGSTRINGS
#error overflow: CS_OBJECTIVES >= MAX_CONFIGSTRINGS
#endif
...
typedef struct gattack_s {
	...
	// Per-shell momentum budget: the maximum resultant knockback-impulse
	// MAGNITUDE one shot of this attack may deliver to a single target in one
	// frame, no matter how many sub-hits (shotgun pellets) it lands. The
	// per-(target,source) bucket's summed impulse is capped to this magnitude at
	// flush time (P_KnockbackFlush). 0 = no budget — single-hit weapons whose one
	// impulse never needs capping leave this zero (rows that omit it). Placed
	// last so existing positional initializers zero-fill it; only momentum-event
	// attacks (shotgun) set a non-zero value. gattack_t is a game+cgame shared,
	// in-process table (it holds function pointers, so it is never serialized to
	// network/savegame) — adding a field is a game-module rebuild, not an ABI
	// break across the VM/protocol boundary.
	float		 shellKnockback;

	// ── Recoil/spread model (RS-1 foundation) ───────────────────────────────
	// Per-attack cone-spread tuning, read by BG_CalcWeaponSpread (bg_weapons.c).
	// These are CONFIGURABLE table values (edit them in bg_attacklist[], not in
	// the formula). Appended at the struct tail so the table's positional
	// initializers zero-fill them for every row that omits them — only the
	// hitscan-cone attacks (machinegun primary/burst, shotgun) set them.
	// A non-cone attack leaves all six 0 ⇒ BG_CalcWeaponSpread returns 0.
	// gattack_t is an in-process game+cgame table (function pointers; never
	// serialized) — adding fields is a VM rebuild, not a netstream ABI break.
	float		 spreadBase;			// cone magnitude at rest (q3 units of MG-style spread)
	float		 spreadCeiling;			// max cone under sustained fire
	float		 spreadRampTime;		// seconds base→ceiling under sustained fire (0 = flat)
	float		 spreadDecayTime;		// seconds current→base after fire stops (0 = instant)

	// View-kick (RS-4 — declared now, 0 this phase; no reader until RS-4).
	float		 viewKickPitch;			// authoritative pitch climb per shot (degrees)
	float		 viewKickRecoverTime;	// seconds to recover the accumulated kick
} gattack_t;
...
#define	BUTTON_ATTACK_PRI	1			// primary fire
#define	BUTTON_ATTACK_SEC	2			// secondary fire (bit 1, slot next to +attackpri)
...
typedef struct playerState_s {
	...
	int			fireRampStartTime;		// RS-1: serverTime anchor for the recoil/spread ramp (0 = at rest).
										// Synced like chargeStartTime so server (cone) and client (crosshair)
										// derive the same spread from BG_CalcWeaponSpread with zero bandwidth.
										// Written by RS-2/RS-3; this phase declares + zero-inits + the formula reads it.
...
typedef struct entityState_s {
	...
	vec3_t	damageDir;		// normalized direction the killing blow came from
							// (gibs launch away along it); {0,0,1} = no bias
} entityState_t;
