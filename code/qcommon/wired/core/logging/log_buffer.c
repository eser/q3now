...
static void LogBuffer_Dump( const char *filename )
{
    ...
    if ( !entries || total == 0 ) {
        COM_INFO( LOG_CH(ch_system), "dumpLogBuffer: nothing captured yet\n" );
        free( entries );
        return;
    }
    ...
    if ( fh == FS_INVALID_HANDLE ) {
        COM_ERROR( LOG_CH(ch_system), "dumpLogBuffer: cannot open '%s' for write\n", filename );
        free( entries );
        return;
    }
    ...
    for ( i = 0; i < total; i++ ) {
        ...
        header_len = Com_sprintf( header, sizeof( header ),
            ...
            ( e->channel >= 0 && e->channel < log_channelCount )
                ? log_channels[ e->channel ].name
                : "general" );
    ...
    FS_FCloseFile( fh );
    COM_INFO( LOG_CH(ch_system), "dumpLogBuffer: wrote %d entries to '%s'\n", total, filename );
...
void LogBuffer_Init( void )
{
    ...
    if ( !Sys_MutexInit( &s_registry_mutex ) ) {
        COM_ERROR( LOG_CH(ch_system), "LogBuffer_Init: failed to create registry mutex\n" );
        return;
    }
