...
// map_q1_bsp.c -- Quake 1 BSP29-format loader
//
// Loader stages:
// - header parse, lump-bounds validation, mapFile_t allocation,
//   entity-string copy, per-lump diagnostic log.
// - planes, shaders (texinfo+miptex), surfaces/drawVerts/
//   drawIndexes (edge-indirect → triangle fan), nodes, leafs,
//   marksurfaces, submodels, embeddedTextures (raw).
// - lightmap atlas packing (4-style shelf packer),
//   per-vertex lightmap UV, vis NULL + leaf cluster=-1,
//   synthetic Q3 rawData so RE_LoadWorldMap works unchanged.
// - clipnode → brush/brushside conversion.
// - miptex expansion + synthetic shader binding.

#include "../q_shared.h"
#include "../qcommon.h"
#include "map_format_registry.h"
#include "../surfaceflags.h"
#include "../cm_public.h"   /* extern cmTracer_q1canon for the format's tracer slot */
#include "../cm_local.h"   /* for cm.q1.leafContents in post-dup extension */
LOG_DECLARE_CHANNEL( ch_loading,    "loading" );
LOG_DECLARE_CHANNEL( ch_loading_q1, "loading.q1" );
                                  ...
                                  int cnIdx )
{
	if ( cnIdx >= 0 ) {
		...
		if ( cnIdx >= numCn ) {
			Com_Log( SEV_WARN, LOG_CH(ch_loading_q1), "BSP_Q1_WalkClipTree: index %d >= numCn %d\n",
			            cnIdx, numCn );
			return;
		}
...
static int Q1_ComputeBrushAABB( const mapFile_t *bsp, const dbrush_t *b,
...
static void Q1_InsertBrushIntoTree( const mapFile_t *bsp, LeafBrushBuckets_t *lb,
                                     int brushIdx,
...
static void BSP_Q1_DiagBrushes( const char *name, const mapFile_t *bsp, int enabled )
{
	...
	Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "=== Brush dump (first %d of %d) ===\n", nPrint, bsp->numBrushes );
	for ( i = 0; i < nPrint; i++ ) {
		const dbrush_t *b = &bsp->brushes[i];
		Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "brush[%d]: %d sides shader=%d\n", i, b->numSides, b->shaderNum );
		for ( k = 0; k < b->numSides; k++ ) {
			const dbrushside_t *s = &bsp->brushSides[b->firstSide + k];
			int pn = s->planeNum;
			const dplane_t *p = &bsp->planes[pn];
			Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "  side[%d]: pn=%d(orig=%d,s=%d) n=(%.4f,%.4f,%.4f) d=%.4f\n",
			            k, pn, pn >> 1, pn & 1,
...

	{
		...
		Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "=== Q3-CM spawn trace ===\n" );
		Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "spawnOrigin=(%.1f,%.1f,%.1f) testPt=spawnOrigin\n",
		...
		while ( nodeIdx >= 0 ) {
			...
			if ( ++walkDepth > 100 ) { Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "BSP walk depth limit\n" ); break; }
		}
		leafIdx = -(nodeIdx + 1);
		Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "render BSP → leafIdx=%d\n", leafIdx );

		if ( leafIdx < 0 || leafIdx >= bsp->numLeafs ) {
			Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "leafIdx out of range (%d leafs)\n", bsp->numLeafs );
			return;
		}

		{
			const dleaf_t *lf = &bsp->leafs[leafIdx];
			Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "leaf[%d]: firstLeafBrush=%d numLeafBrushes=%d\n",
			...
			for ( lb = 0; lb < maxLB; lb++ ) {
				...
				if ( bi < 0 || bi >= bsp->numBrushes ) {
					Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "  leafBrush[%d]: bi=%d OUT OF RANGE\n", lb, bi );
					continue;
				}
				b = &bsp->brushes[bi];
				for ( k = 0; k < b->numSides; k++ ) {
				...
				Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "  leafBrush[%d]=brush[%d] (%d sides): testPt %s\n",
				...
				for ( k = 0; k < b->numSides; k++ ) {
					...
					const dplane_t *p = &bsp->planes[pn];
					float d = DotProduct( testPt, p->normal ) - p->dist;
					Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "    side[%d]: pn=%d(orig=%d,s=%d) n=(%.4f,%.4f,%.4f) dist=%.4f testD=%.4f [%s]\n",
					            k, pn, pn >> 1, pn & 1,
			...
			if ( lf->numLeafBrushes > 8 )
				Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "  ...(%d more brushes)\n", lf->numLeafBrushes - 8 );
		}
	}
}
...
static void BSP_Q1_BuildSyntheticQ3Data( mapFile_t *bsp ) {
...
// ---------------------------------------------------------------------------
static void BSP_Q1_RemoveRedundantPlanes( mapFile_t *bsp ) {
...
/* NOTE: Q1 hull-1 expansion (~56 units Z, 30 units XY) means brushes whose
   hull-1 extent is smaller than the expansion on an axis cannot absorb the full
   de-expansion on that axis without their opposing planes crossing.  Such
   brushes are detected in the inversion check below and skipped from bevel
   emission here; the de-expansion pass (Step 8.5) additionally clamps the
   erosion per axis so the resulting half-space intersection stays non-empty and
   the brush survives CM_BoundBrush.  Before that clamp existed these brushes
   collapsed to zero volume and were dropped from collision entirely — on Q1
   maps, where clip is the only player-collision content, that removed a large
   fraction of every standable surface. */
static void BSP_Q1_AddBrushBevels( mapFile_t *bsp ) {
	int          bi, si, wi;
...
static void BSP_Q1_SynthesizeWorldShell( mapFile_t *bsp )
{
...
static int BSP_Q1_FindOrAddLiquidShader( mapFile_t *bsp, const char *sname,
...
static void BSP_Q1_SynthesizeLiquidBrushes( mapFile_t *bsp,
                                              const byte *base,
...
static void BSP_Q1_PrefixClassnames( mapFile_t *bsp ) {
...
// AABB touch test with 1-unit tolerance — mirrors Q1_MnodesAdjacent in tr_map.c
...
=================
*/
static void Q1_PostProcessPVS( mapFile_t        *bsp,
                                const q1_dleaf_t *rawLeafs ) {
...
static qboolean BSP_Q1_Load( const mapFormat_t *format, const char *name,
	...
	*bspFile = NULL;
	...
	// Lightmap patches are packed into a shelf atlas (4 parallel style pages).
	{
		...
		// Transfer finalised atlas pages to mapFile_t
	...
	// bsp->visibility, which BSP_Q1_BuildSyntheticQ3Data then embeds in the Q3 vis lump.
	{
		...
		if ( !(flags & MAP_LOAD_FLAG_RENDER_ONLY) ) {
	...
	// Each brush's plane set is accumulated on a stack during recursive descent.
	{
		...
		if ( cnLump->filelen % (int)sizeof( q1_dclipnode_t ) ) {
			Com_Log( SEV_WARN, LOG_CH(ch_loading_q1), "BSP_Q1_Load: %s clipnodes lump not aligned; skipping brush conversion\n",
			            name );
			buildBrushes = qfalse;
		}
		...
		if ( buildBrushes ) {
			...
			if ( bsp->numBrushes > 0 && bsp->numNodes > 0 ) {
				...
				// units of a world boundary face.  Interior leaves are untouched.
				{
					...
					if ( boundaryLeafCount == 0 )
						Com_Log( SEV_ERROR, LOG_CH(ch_loading_q1), "BSP_Q1_Load: proximity pass found zero boundary"
				...
				if ( degenCount > 0 )
					Com_Log( SEV_TRACE, LOG_CH(ch_loading_q1), "BSP_Q1_Load: %d degenerate brushes skipped in tree walk\n",
					            degenCount );
			...
			if ( bsp->numBrushSides > 0 ) {
				...
				// Minimum surviving slab thickness for an axis whose hull-1 extent is
				// smaller than the inset it is asked to absorb.  Not a tuning constant:
				// it only has to exceed the float slop of CM_BoundBrush's comparison so
				// the pair stays strictly ordered.  Geometry is unaffected — the slab
				// centre is placed at the world position implied by the same identity.
				#define Q1_DEEXPAND_MIN_SLAB  0.03125f
				...
				// Per-brush, per-axis clamped de-expansion.
				//
				// The de-expansion is a Minkowski *erosion* of the brush by the player
				// bbox.  Erosion is separable per axis: contracting X does not depend on
				// whether Z survives.  So the clamp is applied per axis, not per brush —
				// a brush that is thin only in Z still gets its full, correct X/Y
				// de-expansion, and only the Z pair is preserved.  A whole-brush skip
				// would leave X/Y double-expanded on exactly the thin ledges where the
				// player is most likely to be squeezed, which is the defect this pass
				// exists to remove.
				//
				// For axis k the hull-1 slab is [lo,hi] and the requested erosion is
				//   lo' = lo + q1H1Maxs[k]      (lower face moves up)
				//   hi' = hi + q1H1Mins[k]      (upper face moves down)
				// with total shrink  want = q1H1Maxs[k] - q1H1Mins[k].
				// When (hi-lo) < want the pair crosses.  We then scale *both* faces of
				// that pair by the same factor t = avail/want, avail = (hi-lo) - minSlab,
				// clamped to [0,1].  One shared factor means the surviving slab is the
				// erosion by a proportionally shrunk player box, so each face keeps the
				// same share of the inset it would have taken unclamped; the residual
				// error splits between the two faces in the ratio of their own insets
				// instead of piling entirely onto one of them.  Per-face residual is
				// (1-t)*inset, so it is bounded by the inset itself and goes to zero as
				// the brush approaches the full thickness.
				for ( bi = 0; bi < bsp->numBrushes; bi++ ) {
					const dbrush_t *br = &bsp->brushes[bi];
					float lo[3], hi[3];
					float scale[3];
					int   k;
					int   sEnd = br->firstSide + br->numSides;

					if ( br->numSides <= 0 ) continue;

					// Axial extent of this brush in hull-1 space.  Non-axial sides
					// contribute nothing here; a missing axial pair leaves the axis
					// unconstrained, which correctly yields scale = 1 (no crossing
					// possible without an opposing plane).
					for ( k = 0; k < 3; k++ ) {
						lo[k]    = -65536.0f;
						hi[k]    =  65536.0f;
						scale[k] =  1.0f;
					}
					for ( si = br->firstSide; si < sEnd; si++ ) {
						const dbrushside_t *s = &bsp->brushSides[si];
						const dplane_t     *op;
						if ( !( bsp->shaders[s->shaderNum].contentFlags &
						        ( CONTENTS_SOLID | CONTENTS_PLAYERCLIP ) ) )
							continue;
						op = &bsp->planes[s->planeNum];
						for ( k = 0; k < 3; k++ ) {
							if ( op->normal[k] > 0.999f ) {
								if ( op->dist < hi[k] ) hi[k] = op->dist;
							} else if ( op->normal[k] < -0.999f ) {
								if ( -op->dist > lo[k] ) lo[k] = -op->dist;
							}
						}
					}
					for ( k = 0; k < 3; k++ ) {
						float want, extent, avail;
						if ( lo[k] <= -65536.0f || hi[k] >= 65536.0f )
							continue;                       // axis unbounded — nothing to cross
						want   = q1H1Maxs[k] - q1H1Mins[k];
						if ( want <= 0.0f ) continue;
						extent = hi[k] - lo[k];
						if ( extent >= want ) continue;     // pair stays ordered — full erosion
						avail  = extent - Q1_DEEXPAND_MIN_SLAB;
						if ( avail < 0.0f ) avail = 0.0f;
						scale[k] = avail / want;
					}

					// Emit the de-expanded planes for this brush.
					for ( si = br->firstSide; si < sEnd; si++ ) {
						dbrushside_t   *s = &bsp->brushSides[si];
						const dplane_t *op;
						float offset = 0.0f;
						// Only hull-1 clip brushes need de-expansion; liquid/water
						// brushes are already in world space and must not be contracted.
						if ( !( bsp->shaders[s->shaderNum].contentFlags &
						        ( CONTENTS_SOLID | CONTENTS_PLAYERCLIP ) ) )
							continue;
						op = &bsp->planes[s->planeNum];
						for ( k = 0; k < 3; k++ )
							offset += op->normal[k] * scale[k] *
...
/* -------------------------------------------------------------------------
   Mover-submodel phantom-floor exclusion (Q1 nav surface pass)

   The Q1 loader emits one draw-surface per compiled BSP face and tags any
   non-liquid/non-sky/non-alpha texture as CONTENTS_SOLID, so brush-entity
   up-faces (trigger volumes, doors, plats, buttons) rasterize as walkable
   "floor" in the nav surface pass — even though a trigger volume is not a
   floor a bot ever stands on. That phantom floor fabricates false vertical
   deltas (e.g. e1m1's corridor->exit doorway, where a trigger_secret top
   sits ~32u above the real corridor floor) that exceed walkableClimb and
   sever the mesh.

   The hull-1 clip-brush pass (below) already carries the REAL floor: it
   iterates submodel-0 brushes only, so the honest collision geometry is
   untouched by this filter. This exclusion is purely on the visual surface
   pass, and it is SUBTRACTIVE — it only ever drops phantom surfaces, so it
   can never over-connect the mesh; the only risk is under-connecting by
   dropping a solid mover top a bot legitimately stands on, which the
   entity-class gate below is designed to avoid.

   Class gate (Eser-ratified Option-2 — derived from the entity class, not a
   hardcoded submodel number):
     - Submodel 0 (worldspawn)  -> KEEP (the real world floor).
     - trigger_* brush entities -> DROP. Q1 triggers are non-solid volumes
       (SOLID_TRIGGER); a bot passes through them and never stands on their
       up-faces, so those faces are pure phantom floor.
     - func_* solid movers (func_plat, func_door, func_door_secret,
       func_button, func_wall, ...) -> KEEP. Their at-rest tops can be
       legitimate standable geometry (a plat top a bot rides, a low door
       used as a step), so preserving them keeps real routes connected.
       Conservative default: anything not recognised as a trigger is kept.
   ------------------------------------------------------------------------- */

/* Look up the classname of the brush entity whose "model" key is "*idx".
   Reuses the same entity-block scan the mover/door paths use. Writes the
   classname (possibly "q1_"-prefixed) into out; empty string if not found
   (e.g. submodel 0 / worldspawn, which has no "*0" model reference). */
static void BSP_Q1_ClassnameForSubmodel( const mapFile_t *bsp, int idx,
                                          char *out, int outLen )
{
	const char *p;

	if ( out && outLen > 0 ) out[0] = '\0';
	if ( !bsp || !bsp->entityString || idx <= 0 || !out || outLen <= 0 )
		return;
	...
	while ( *p ) {
		...
		if ( modelIdx == idx ) {
			Q_strncpyz( out, classname, outLen );
			return;
		}
	}
}

/* True if the given submodel's owning brush entity is a non-solid trigger
   volume whose walkable up-faces are phantom floor and must be excluded from
   the nav surface pass. Worldspawn (submodel 0) and solid func_* movers are
   never excluded (conservative). Classnames arrive "q1_"-prefixed from
   BSP_Q1_PrefixClassnames, so strip that prefix before matching. */
static qboolean BSP_Q1_SubmodelIsExcludedMover( const mapFile_t *bsp, int idx )
{
	char classname[64];
	const char *cn;

	if ( idx <= 0 )
		return qfalse; /* submodel 0 = worldspawn: always keep */

	BSP_Q1_ClassnameForSubmodel( bsp, idx, classname, sizeof(classname) );
	if ( !classname[0] )
		return qfalse; /* no owning brush entity found: keep (conservative) */

	cn = classname;
	if ( Q_stricmpn( cn, "q1_", 3 ) == 0 )
		cn += 3; /* strip the Q1 format prefix; compare the bare classname */

	/* trigger_* = non-solid volume, never a floor -> exclude. Everything
	   else (func_* solid movers, unknown) -> keep. */
	return ( Q_stricmpn( cn, "trigger_", 8 ) == 0 ) ? qtrue : qfalse;
}

/* Return true if the draw-surface at global index surfIdx belongs to a
   brush-entity submodel that is an excluded (trigger) mover. Determines the
   owning submodel from the MODELS lump face ranges (firstSurface/numSurfaces;
   surface index == Q1 face index). Faces outside every submodel range, or in
   submodel 0, are kept. */
static qboolean BSP_Q1_SurfaceIsExcludedMover( const mapFile_t *bsp, int surfIdx )
{
	int m;
	...
	/* Submodel 0 owns the leading contiguous face range (worldspawn). Skip it
	   explicitly — its faces are always kept. */
	for ( m = 1; m < bsp->numSubModels; m++ ) {
		const dmodel_t *sm = &bsp->subModels[m];
		if ( sm->numSurfaces <= 0 ) continue;
		if ( surfIdx >= sm->firstSurface &&
		     surfIdx <  sm->firstSurface + sm->numSurfaces ) {
			return BSP_Q1_SubmodelIsExcludedMover( bsp, m );
		}
	}
	return qfalse;
}

static qboolean BSP_Q1_ExtractNavGeometry( const mapFile_t  *bsp,
                                            struct navGeom_s *outGeom )
{
	...
	for ( i = 0; i < bsp->numSurfaces; i++ ) {
		...
		/* Exclude phantom floor from non-solid mover (trigger) submodels;
		   worldspawn and solid func_* mover tops are kept. Surface pass only —
		   the hull-1 clip pass below carries the real floor untouched. */
		if ( BSP_Q1_SurfaceIsExcludedMover( bsp, i ) )
			continue;
	...
	for ( i = 0; i < bsp->numSurfaces; i++ ) {
		...
		/* Mover phantom-floor exclusion — must mirror the Pass 1 count gate
		   exactly so verts/tris allocation stays consistent. */
		if ( BSP_Q1_SurfaceIsExcludedMover( bsp, i ) ) {
			moverSurfExcluded++;
			continue;
		}
	...
	Com_Log( SEV_INFO, LOG_CH(ch_nav_build),
	         "%s: Q1 surface pass excluded %d mover (trigger) phantom-floor surfaces\n",
	         bsp->name, moverSurfExcluded );
	}
	...
	/* --- Pass 2 (brushes): emit triangles from hull-1 clip geometry --- */
	{
		...
		Com_Log( SEV_INFO, LOG_CH(ch_nav_build), "%s: Q1 brush extraction skipped %d brushes (shell+non-walkable)\n",
		         bsp->name, skipped );
	}
	 ...
	 * Must run BEFORE rcCalcBounds so inflated coords never reach Recast. */
	{
		...
		if ( dropped > 0 ) {
			Com_Log( SEV_WARN, LOG_CH(ch_nav_build),
			    "%s: dropped %d degenerate triangles (first bad vert: %.1f %.1f %.1f)\n",
...
/* -------------------------------------------------------------------------
   BSP_Q1_ExtractMovers — Q1-format vertical-mover descriptor extraction.

   Recognizes the Q1 elevator platform. The Q1 BSP loader rewrites every
   non-worldspawn classname to a "q1_"-prefixed form (BSP_Q1_PrefixClassnames),
   so the platform arrives here as "q1_func_plat". Classname recognition reuses
   the same "q1_"-strip rule as the door path (Nav_Get_DoorBoxes'
   DoorEnt_ClassnameIs) — strip an optional "q1_" prefix, then compare the bare
   name — rather than inventing a new scheme.

   Endpoint geometry comes from the SUBMODEL AABB, NOT the entity origin key.
   Q1 brushmodel entities carry origin=(0,0,0) and encode world position in the
   compiled submodel bounds (like the door path, which reads dm->mins/maxs).
   The brush is compiled at its rest/top position, so:
     topLedge.z   = dm->maxs[2]                 (plat top surface at the upper stop)
     shaftFloor.z = dm->maxs[2] - travelHeight  (top surface at the lower stop)
   travelHeight mirrors SP_q1_func_plat: the "height" key if present, else
   fabsf(maxs.z - mins.z) - lip, lip defaulting to 8. x/y are the AABB centre.
   ------------------------------------------------------------------------- */

/* Format-agnostic classname match on a raw parsed entity value. Mirrors the
 * door path's DoorEnt_ClassnameIs: a Q1 map's classnames arrive "q1_"-prefixed,
 * so strip an optional "q1_" and compare the bare name. Static/local — the same
 * one-line rule the door recognizer uses, reproduced here (it is not shared). */
static qboolean Q1MoverEnt_ClassnameIs( const char *classname, const char *bareName ) {
...
static int BSP_Q1_ExtractMovers( const mapFile_t *bsp,
	...
	while ( *p && count < maxOut ) {
		...
		/* Scan key/value pairs within this entity block. */
		kp = bstart + 1;
		while ( kp < bend ) {
			char key[64] = "";
			char val[256] = "";
		...
		if ( !Q1MoverEnt_ClassnameIs( classname, "func_plat" ) )
			continue;
...

		{
			...
			float czHigh = dm->maxs[2]; /* plat top surface at the rest/upper stop */
			float travel;

			/* Travel height mirrors SP_q1_func_plat (g_mover_q1.c): explicit
			 * "height" key overrides, else brush Z span minus lip (default 8). */
			if ( heightVal[0] ) {
				travel = (float)atof( heightVal );
			} else {
				float lip = lipVal[0] ? (float)atof( lipVal ) : 8.0f;
				travel = fabsf( dm->maxs[2] - dm->mins[2] ) - lip;
			}
			if ( travel <= 0.0f )
			...
			out[count].shaftFloor[2] = czHigh - travel;
			out[count].radius        = 32.0f;
			out[count].kind          = NAV_MOVER_PLATFORM;

			Com_Log( SEV_DEBUG, LOG_CH(ch_nav_build),
			    "%s: q1_func_plat mover model=*%d top=(%.0f,%.0f,%.0f) floor=(%.0f,%.0f,%.0f) travel=%.0f\n",
			    bsp->name, modelIdx, cx, cy, czHigh, cx, cy, czHigh - travel, travel );
			count++;
		}
	}
...
const mapFormat_t bspFormatQ1 = {
	...
	BSP_Q1_ExtractMovers,
#else
	NULL,
	NULL,
#endif
	&cmTracer_q1canon	// Q1 maps collide via the canonical(+clip) brush model
};
