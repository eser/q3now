...
#define MAX_ATTACHMENTS_IN_POOL (8+(VK_NUM_BLOOM_PASSES+1)+3) // bloom pyramid = 1 full + N mips; +3 for SMAA edges/blend/input when active
...
//   - set 7 (binding 0) = SAMPLED_IMAGE[WIRED_BINDLESS_TEX_SLOTS] table
//   - set 7 (binding 1) = SAMPLER[WIRED_BINDLESS_SAMPLER_SLOTS] table
//   - FS push at offset WIRED_BINDLESS_INDEX_BLOCK_OFFSET carries
//     WIRED_BINDLESS_INDEX_COUNT × uint32, each packed as
//     (sampler:WIRED_BINDLESS_SAMPLER_BITS << WIRED_BINDLESS_TEX_BITS) | tex.
...
// Push-constant layout (host writers and pipeline_layout share these constants):
//   VS [0, 64)              mat4 mvp
//   FS [64, 96)             fog block (32 B; populated only when FEAT_FOG_SYSTEM)
//   FS [96, 128)            (retired) bindless index block — migrated into the
//                           set-0 UBO (vkUniform_t::packed_indices @ offset 544).
//                           The fragment shaders read the per-draw bindless table
//                           from the UBO; no fragment stage reads push constants
//                           when FEAT_FOG_SYSTEM is off. The push-constant ceiling is
//                           RESOLVED: this migration freed 32 B of push room and,
//                           more importantly, an extensible UBO table (12 roles,
//                           roles 8..11 reserved — 8 = pbrMap for the base-pass
//                           IBL term). WebGPU (no push constants) reads it natively.
//   There is NO vertex-stage push range anywhere in the renderer. Every vertex
//   shader reads mvp from the set-0 UBO (vkUniform_t::mvp). The last VS push range —
//   the halo dot-probe's MVP push on vk.pipeline_layout_storage — was retired with
//   the dot-probe occlusion machinery. So vk.pipeline_layout carries ONLY the fog FS
//   range [64, 96) when FEAT_FOG_SYSTEM is on, and ZERO push ranges when off. The
//   fog block still starts at offset 64 (the slot the retired VS MVP range used to
//   occupy). The WIRED_BINDLESS_INDEX_* constants still bound the host packing
//   loop (vk_push_bindless_indices).
#define WIRED_FOG_PUSH_OFFSET          64 /* fog FS block; sits where the retired VS MVP push range was */
#define WIRED_FOG_PUSH_SIZE            32
#define WIRED_BINDLESS_INDEX_BLOCK_SIZE   (WIRED_BINDLESS_INDEX_COUNT * (int)sizeof(uint32_t)) /* vestigial: bindless block migrated off push into the set-0 UBO (packed_indices[12]@544) */
#define WIRED_BINDLESS_INDEX_BLOCK_OFFSET (WIRED_FOG_PUSH_OFFSET + WIRED_FOG_PUSH_SIZE)    /* 96 */
...
typedef struct VK_Pipeline {
	...
	// Dynamic-rendering sibling of handle[]. Built for the world-pass slots —
	// RENDER_PASS_MAIN (3D world + UI piggyback it) and RENDER_PASS_SCREENMAP
	// (mirror/portal); both share the color/depth formats. NULL for POST_BLOOM,
...
// vk_ral_adopt_static_internal_textures (vk_ral_textures.c) once the color
...
// Symmetric teardown, called from vk_ral_destroy_adopted_internal_textures so
...
// WIRED_BINDLESS_INDEX_BLOCK_OFFSET. Both run
...
typedef struct vk_tess_s {
	...
	// reference the alias unchanged (bypass-state-tracker rule: the
	...
	struct {
		...
		// vk_bind_descriptor_sets can fold it into the per-draw batched
		// qvkCmdBindDescriptorSets call — Vulkan's pipeline-layout-compat
	 ...
	 * them), so vk_bind_pipeline dedups world binds on this handle instead.
	 * Reset to NULL in lockstep with last_pipeline (pass-begin / cleanup sites) so
	 * the first world draw of each pass re-binds after Ral_BeginRendering. */
	 ...
	 * Set/cleared in lockstep with last_pipeline at every bind. */
	...
	VkBuffer        shadowEntMatBuf;
	VkDeviceMemory  shadowEntMatMem;
...
typedef struct {
	...
	struct {
		// main + screenmap are dynamic-rendering now (no legacy VkRenderPass).
		// tonemap is dynamic-rendering only (writes tonemapped_image) — no VkRenderPass.
	...
	// Typed RAL siblings (Ral_AdoptPipelineLayout wrappers).
	...
	struct {
		...
		// writes). Indexed by ping-pong bit, NOT by
		...
		// shadow registry: RE_RegisterParticleClass writes
	...
	struct {
		// pool ring (host-coherent for emit-time CPU writes)
		VkBuffer				pool_buffer;
		VkDeviceMemory			pool_memory;
		byte					*pool_ptr;
		...
		// per-frame uniform buffer (host-coherent, one slot per cmd_index)
		VkBuffer				frame_buffer [NUM_COMMAND_BUFFERS];
		VkDeviceMemory			frame_memory [NUM_COMMAND_BUFFERS];
		byte					*frame_ptr   [NUM_COMMAND_BUFFERS];
	...
	// Mirrors vk.particle: ping-pong pool SSBOs (host-coherent), a per-frame
	...
	// frame. Gated by r_atmosphericGPU (0 = skip). RAL-native (buffers registered
	// via vk_ral_register_buffer, render pipeline via the gpinfo translator).
	struct {
		// pipeline state
		...
		// ping-pong pool (host-coherent; memset-zeroed at init so every slot
		// starts inactive, which the compute respawns on its first pass).
		VkBuffer				pool_buffer  [2];
		VkDeviceMemory			pool_memory  [2];
		byte					*pool_ptr    [2];
		...
		// Heightgrid data texture (R32F 256×256 ground heights). Dedicated
		// VkImage + memory + view + sampler (SMAA-LUT idiom). Created eagerly
		// at init (zeroed) so the compute descriptor always binds a valid
		// image; RE_SetAtmosphereHeightgrid re-uploads the cgame tracemap into
		// it on weather / map change. Survives vid_restart (dedicated memory).
		VkImage					heightgrid_image;
		VkDeviceMemory			heightgrid_memory;
		VkImageView				heightgrid_view;
	...
	struct {
		...
		VkBuffer				bone_buffer[NUM_COMMAND_BUFFERS]; // per-frame bones+mvp ring
		...
		VkDeviceSize			ring_size;			// per-frame ring buffer size in bytes
	...
	struct {
		...
		// instead of a raw qvkCmdBindDescriptorSets. Adopted (ownsBuffer/ownsSet
	...
	// bound via the raw qvkCmdBindDescriptorSets path in vk_bind_descriptor_sets
	// (no RAL bindgroup adoption needed — only the layout adoption auto-reflects
	// the 4th set).
	...
	// ring above but bound by the RAW qvkCmdBindDescriptorSets path at the draw site
	// (firstSet=1) — the effects never used the descriptor_set.current[] fold, so no
	...
	qboolean                     exposureAccumulatorSeeded;           // false until the one-time 1.0 fill is recorded
	...
	VkImage                      fpDepthImage;           // depth copy (TRANSFER_DST|SAMPLED, vk.depth_format)
	VkImageView                  fpDepthView;            // depth-only-aspect sampling view (raw vk, for adopt)
	VkDeviceMemory               fpDepthMemory;
	struct ralSampler_s         *ral_fpDepthSampler;    // nearest, clamp (reduce reads exact texels)
	struct ralTexture_s         *ral_fpDepthImage;      // adopted RAL sibling (copy dst + transition tracking)
	...
	VkBuffer                     fpClusterParamsBuf;    // grid-params UBO {origin.xyz, cellSize; dims.xyz}
	VkDeviceMemory               fpClusterParamsMem;
...
#define LENS_SOURCE_VEC4S  2                 // 2 vec4 per record (32 B): rec0 pos/depth, rec1 vis
	...
	VkImage bloom_image[1+VK_NUM_BLOOM_PASSES];
	VkImageView bloom_image_view[1+VK_NUM_BLOOM_PASSES];
	// Adopted dynamic-rendering siblings for the bloom extract/downsample/upsample
	...
	// vk_ral_adopt_static_internal_textures; bound as Ral_BeginRendering color
	...
	struct ralTexture_s *ral_depth_image;       // adopted typed sibling
	...
	struct {
		...
		struct ralTexture_s *ral_image;  // adopted typed sibling for typed Ral_Cmd{PipelineBarrierFull,CopyImage} (closes the lookup-miss gap at vk.c:18411/18448/...) AND the bindless slot read (Ral_BindGroupSetTextureAt) — re-adopted each vk_update_attachment_descriptors so it never wraps a freed view
	...
	struct {
		...
		// layerViews = `layerView[]` (per-cascade attachment). Ral_BeginRendering
		// selects layerView[depthAttachmentLayerIndex] for each cascade.
		...
		VkBuffer         cascadeMvpBuf[NUM_COMMAND_BUFFERS];
		VkDeviceMemory   cascadeMvpMem[NUM_COMMAND_BUFFERS];
		...
		VkBuffer         casterAtestBuf;       // [stride-32 verts][uint32 indices], device-local
		VkDeviceMemory   casterAtestMem;
		...
		// vk.iqmGpu.bone_buffer); each captured caster sub-allocates a PAD-aligned
		...
		VkBuffer                boneBuf[NUM_COMMAND_BUFFERS];   // per-frame bone ring
		VkDeviceMemory          boneMem[NUM_COMMAND_BUFFERS];
	...
	struct {
		VkImage          image;        // 2D depth atlas (6*K face tiles wide), TRANSFER/SAMPLED depth-format
		VkImageView      view;         // 2D sampling view (whole atlas) — bound for the lit-shader sample
		VkImageView      faceView[6];  // per-face 2D views (one tile each) — render-target attachments
		struct ralTexture_s *ral_image;// adopted array-of-faces sibling (per-face attachment + whole-atlas sample)
		VkDeviceMemory   memory;
		...
		VkBuffer         faceMvpBuf[NUM_COMMAND_BUFFERS];
		VkDeviceMemory   faceMvpMem[NUM_COMMAND_BUFFERS];
		...
		VkBuffer         entMatBuf[NUM_COMMAND_BUFFERS];   // 1-slot identity model→world (worldspawn casters)
		VkDeviceMemory   entMatMem[NUM_COMMAND_BUFFERS];
		...
		VkBuffer         paramsBuf[NUM_COMMAND_BUFFERS];
		VkDeviceMemory   paramsMem[NUM_COMMAND_BUFFERS];
	...
	struct {
		...
		struct ralTexture_s *ral_depth_image;   // adopted sibling — dynamic-rendering depth(+stencil) attachment
	...
	struct {
		...
		struct ralTexture_s *ral_image;   // adopted sibling — dynamic-rendering capture target (adopted at creation; supersample-gated)
	} capture;

	struct {
		// main[] / bloom_extract / blur are dynamic-rendering now (they render into
		// RAL-adopted images), so no legacy framebuffers.
		// tonemap is dynamic-rendering only (writes tonemapped_image) — no framebuffer.
	...
	struct {
		...
		VkShaderModule atmospheric_integrate_cs;
...
#define TONEMAP_VAR_COUNT   16
	...
	qboolean depthClampSupported;   // RAL caps: device VkPhysicalDeviceFeatures.depthClamp enabled.
	                                // qfalse → no native rasterizer depth-clamp (WebGL2) → the
	...
	// Per-GPU BCn texture format support. Populated at
