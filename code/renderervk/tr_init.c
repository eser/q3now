...
#ifdef USE_VULKAN
	ri.Log( SEV_INFO, "\n" );
	ri.Log( SEV_INFO, "VK_VENDOR: %s\n", glConfig.vendor_string );
	ri.Log( SEV_INFO, "VK_RENDERER: %s\n", glConfig.renderer_string );
	ri.Log( SEV_INFO, "VK_VERSION: %s\n", glConfig.version_string );

	if ( vk.driverNote[0] != '\0' )
	{
		ri.Log( SEV_INFO, "%s", vk.driverNote );
	}

	ri.Log( SEV_INFO, "\n" );
	ri.Log( SEV_INFO, "VK_MAX_TEXTURE_SIZE: %d\n", glConfig.maxTextureSize );
	ri.Log( SEV_INFO, "VK_MAX_TEXTURE_UNITS: %d\n", glConfig.numTextureUnits );
#else
...
static void VkInfo_f( void )
{
	ri.Log( SEV_INFO, "max_vertex_usage: %iKb\n", (int)((vk.stats.vertex_buffer_max + 1023) / 1024) );
	ri.Log( SEV_INFO, "max_push_size: %ib\n", vk.stats.push_size_max );

	ri.Log( SEV_INFO, "pipeline handles: %i\n", vk.pipeline_create_count );
	ri.Log( SEV_INFO, "pipeline descriptors: %i, base: %i\n", vk.pipelines_count, vk.pipelines_world_base );
	ri.Log( SEV_INFO, "image chunks: %i\n", vk_world.num_image_chunks );
}
#endif
...
static void R_Register( void )
{
...
#if FEAT_SSAO
	r_ssao = ri.Cvar_Get( "r_ssao", "0", CVAR_ARCHIVE | CVAR_NODEFAULT | CVAR_LATCH );
...
#if FEAT_GODRAYS
	r_godRays = ri.Cvar_Get( "r_godRays", "0", CVAR_ARCHIVE | CVAR_NODEFAULT | CVAR_LATCH );
	...
	r_sunX = ri.Cvar_Get( "r_sunX", "0.5", CVAR_ARCHIVE | CVAR_NODEFAULT );
