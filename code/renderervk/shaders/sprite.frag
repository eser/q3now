...
ready) author colours at full intensity. The linear-pipeline
migration removed the legacy CGEN_VERTEX halving from
...
slot is dead (host writes 1.0f for layout invariance); a later change
removes the field.
...
// Declaration must match sprite.vert's set-1 effects UBO block (no field read here).
...
void main() {
	...
	// decode the per-vertex colour to linear domain
