...
fn main_1() {
    ...
    if override_type_10_3 {
        ...
        if override_type_10_4 {
            ...
            if override_type_10_5 {
                let _e110 = base[3u];
                let _e112 = fog[3u];
                base[3u] = (_e110 * (1f - _e112));
            } else {
                let _e116 = base;
                let _e117 = fog;
                let _e119 = unnamed.fogColor;
                let _e122 = fog[3u];
                base = mix(_e116, (_e117 * _e119), vec4(_e122));
            }
        }
    }
    if override_type_10_6 {
        ...
        if override_type_10_7 {
            let _e128 = base;
            let _e130 = base;
            if (dot(_e128.xyz, _e130.xyz) == 0f) {
                discard;
            }
        }
    }
    let _e134 = gl_FragCoord_1;
    let _e139 = unnamed.packed_indices[1i][0u];
    let _e145 = unnamed.packed_indices[1i][0u];
    let _e150 = textureDimensions(wired_bindless_images[(_e139 & 4095u)], 0i);
    screenUV = (_e134.xy / vec2<f32>(vec2<i32>(_e150)));
    let _e157 = unnamed.packed_indices[1i][0u];
    let _e163 = unnamed.packed_indices[1i][0u];
    let _e168 = screenUV;
    let _e169 = textureSample(wired_bindless_images[(_e157 & 4095u)], wired_bindless_samplers[((_e163 >> bitcast<u32>(12i)) & 255u)], _e168);
    sceneDepth = _e169.x;
    let _e172 = gl_FragCoord_1[2u];
    fragDepth = _e172;
    let _e173 = fragDepth;
    let _e174 = sceneDepth;
    depthDiff = (_e173 - _e174);
    let _e177 = depthDiff;
    fadeFactor = smoothstep(0f, (depth_fade_scale * 0.0005f), _e177);
    let _e179 = fadeFactor;
    let _e181 = base[3u];
    base[3u] = (_e181 * _e179);
    let _e184 = fadeFactor;
    let _e185 = base;
    let _e187 = (_e185.xyz * _e184);
    base[0u] = _e187.x;
    base[1u] = _e187.y;
    base[2u] = _e187.z;
    let _e194 = base;
