...
fn main_1() {
    ...
    let _e73 = frag_tex_coord_1;
    parallax_tc = _e73;
    let _e74 = N_1;
    Np = _e74;
    param_1 = 0u;
    let _e75 = parallax_tc;
    param_2 = _e75;
    param_3 = 0i;
    let _e76 = wired_bl_sample_domain_u0028_u1_u003b_vf2_u003b_i1_u003b((&param_1), (&param_2), (&param_3));
    base = _e76;
    if override_type_22_ {
        if override_type_22_1 {
            let _e78 = base[3u];
            base[3u] = select(0f, 1f, (_e78 > 0f));
        } else {
            if override_type_22_2 {
                let _e83 = base[3u];
                param_4 = alpha_test_value;
                param_5 = (1f - _e83);
                let _e85 = frag_tex_coord_1;
                param_6 = _e85;
                let _e86 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_4), (&param_5), (&param_6));
                base[3u] = _e86;
            } else {
                if override_type_22_3 {
                    ...
                    let _e89 = base[3u];
                    param_8 = _e89;
                    let _e90 = frag_tex_coord_1;
                    param_9 = _e90;
                    let _e91 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_7), (&param_8), (&param_9));
                    base[3u] = _e91;
                }
            }
        }
    ...
    let _e103 = unnamed.lightColor;
    lightColorRadius = _e103;
    let _e104 = L_1;
    let _e108 = unnamed.lightVector;
    let _e113 = unnamed.lightVector[3u];
    scale_1 = clamp((dot(-(_e104.xyz), _e108.xyz) * _e113), 0f, 1f);
    let _e117 = unnamed.lightVector;
    let _e118 = scale_1;
    let _e120 = L_1;
    LL = ((_e117 * _e118) + _e120);
    let _e122 = LL;
    nL = normalize(_e122.xyz);
    let _e125 = V_1;
    nV = normalize(_e125.xyz);
    let _e128 = LL;
    let _e130 = LL;
    let _e134 = lightColorRadius[3u];
    intensFactor = (1f - (dot(_e128.xyz, _e130.xyz) * _e134));
    let _e137 = intensFactor;
    if (_e137 <= 0f) {
        discard;
    }
    let _e139 = lightColorRadius;
    let _e141 = intensFactor;
    intens = (_e139.xyz * _e141);
    let _e143 = Np;
    let _e144 = nL;
    diffuse = dot(_e143, _e144);
    let _e146 = Np;
    let _e147 = nL;
    let _e148 = nV;
    specFactor = dot(_e146, normalize((_e147 + _e148)));
    if override_type_22_7 {
        let _e152 = diffuse;
        let _e153 = Np;
        let _e154 = nV;
        if ((_e152 * dot(_e153, _e154)) <= 0f) {
            discard;
        }
        let _e158 = diffuse;
        diffuse = abs(_e158);
        let _e160 = specFactor;
        specFactor = abs(_e160);
    } else {
        let _e162 = diffuse;
        diffuse = max(_e162, 0f);
        let _e164 = specFactor;
        specFactor = max(_e164, 0f);
    }
    let _e166 = specFactor;
    let _e170 = base;
    spec = ((vec4((pow(_e166, 10f) * 0.25f)) * _e170) * 0.8f);
    let _e173 = base;
    let _e174 = diffuse;
    let _e177 = spec;
    let _e179 = intens;
    out_color = (((_e173 * vec4(_e174)) + _e177) * vec4<f32>(_e179.x, _e179.y, _e179.z, 1f));
    return;
}
