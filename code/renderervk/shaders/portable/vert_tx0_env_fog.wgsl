...
fn main_1() {
    ...
    viewer = normalize((_e54.xyz - _e56));
    let _e59 = in_normal_1;
    let _e60 = viewer;
    d = dot(_e59, _e60);
    let _e62 = in_normal_1;
    let _e65 = d;
    let _e67 = viewer;
    reflected = (((_e62.yz * 2f) * _e65) - _e67.yz);
    let _e71 = reflected[0u];
    frag_tex_coord0_[0u] = (0.5f + (_e71 * 0.5f));
    let _e76 = reflected[1u];
    frag_tex_coord0_[1u] = (0.5f - (_e76 * 0.5f));
...
fn main(@builtin(instance_index) gl_InstanceIndex: u32, @location(0) in_position: vec3<f32>, @location(1) in_color0_: vec4<f32>, @location(5) in_normal: vec3<f32>) -> VertexOutput {
    ...
    let _e18 = frag_color0_;
    let _e19 = frag_tex_coord0_;
