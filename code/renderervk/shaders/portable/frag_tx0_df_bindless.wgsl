...
fn main_1() {
    ...
    param_1 = 0u;
    let _e50 = frag_tex_coord0_1;
    param_2 = _e50;
    param_3 = 0i;
    let _e51 = wired_bl_sample_domain_u0028_u1_u003b_vf2_u003b_i1_u003b((&param_1), (&param_2), (&param_3));
    color0_ = _e51;
    let _e52 = color0_;
    base = _e52;
    if override_type_10_ {
        let _e54 = color0_[3u];
        if (_e54 == alpha_test_value) {
            discard;
        }
    } else {
    ...
    if override_type_10_3 {
        ...
        if override_type_10_4 {
            let _e66 = base;
            let _e68 = base;
            if (dot(_e66.xyz, _e68.xyz) == 0f) {
                discard;
            }
        }
    }
    let _e73 = base[3u];
    if (_e73 < depth_fragment) {
        discard;
    }
...
@fragment 
fn main(@location(1) frag_tex_coord0_: vec2<f32>, @builtin(position) gl_FragCoord: vec4<f32>) -> @builtin(frag_depth) f32 {
