...
fn main_1() {
    ...
    let _e102 = unnamed.packed_indices[0i][1u];
    let _e108 = unnamed.packed_indices[0i][1u];
    let _e113 = fog_tex_coord_1;
    let _e114 = textureSample(wired_bindless_images[(_e102 & 4095u)], wired_bindless_samplers[((_e108 >> bitcast<u32>(12i)) & 255u)], _e113);
    fog = _e114;
    ...
    if override_type_20_2 {
        if override_type_20_3 {
            let _e120 = base[3u];
            base[3u] = select(0f, 1f, (_e120 > 0f));
        } else {
            if override_type_20_4 {
                let _e125 = base[3u];
                param_8 = alpha_test_value;
                param_9 = (1f - _e125);
                let _e127 = frag_tex_coord_1;
                param_10 = _e127;
                let _e128 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_8), (&param_9), (&param_10));
                base[3u] = _e128;
            } else {
                if override_type_20_5 {
                    ...
                    param_13 = _e132;
                    let _e133 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_11), (&param_12), (&param_13));
                    base[3u] = _e133;
                }
            }
        }
    ...
    let _e145 = unnamed.lightColor;
    lightColorRadius = _e145;
    let _e146 = L_1;
    nL = normalize(_e146.xyz);
    let _e149 = V_1;
    nV = normalize(_e149.xyz);
    let _e152 = L_1;
    let _e154 = L_1;
    let _e158 = lightColorRadius[3u];
    intensFactor = (1f - (dot(_e152.xyz, _e154.xyz) * _e158));
    let _e161 = intensFactor;
    if (_e161 <= 0f) {
        discard;
    }
    let _e163 = lightColorRadius;
    let _e165 = intensFactor;
    intens = (_e163.xyz * _e165);
    dbgCascade = 0i;
    let _e167 = shadowData_1;
    param_14 = _e167.xyz;
    let _e170 = shadowData_1[3u];
    param_15 = _e170;
    let _e171 = sampleShadow_u0028_vf3_u003b_f1_u003b_i1_u003b((&param_14), (&param_15), (&param_16));
    let _e172 = param_16;
    dbgCascade = _e172;
    let _e173 = intens;
    intens = (_e173 * _e171);
    ...
    let _e187 = Np;
    let _e188 = nL;
    diffuse = dot(_e187, _e188);
    let _e190 = Np;
    let _e191 = nL;
    let _e192 = nV;
    specFactor = dot(_e190, normalize((_e191 + _e192)));
    if override_type_20_9 {
        let _e196 = diffuse;
        let _e197 = Np;
        let _e198 = nV;
        if ((_e196 * dot(_e197, _e198)) <= 0f) {
            discard;
        }
        let _e202 = diffuse;
        diffuse = abs(_e202);
        let _e204 = specFactor;
        specFactor = abs(_e204);
    } else {
        let _e206 = diffuse;
        diffuse = max(_e206, 0f);
        let _e208 = specFactor;
        specFactor = max(_e208, 0f);
    }
    let _e210 = specFactor;
    let _e214 = base;
    spec = ((vec4((pow(_e210, 10f) * 0.25f)) * _e214) * 0.8f);
    let _e217 = base;
    let _e218 = diffuse;
    let _e221 = spec;
    let _e223 = intens;
    out_color = (((_e217 * vec4(_e218)) + _e221) * vec4<f32>(_e223.x, _e223.y, _e223.z, 1f));
    let _e231 = unnamed.eyePos[3u];
    if (_e231 > 0.5f) {
        let _e233 = dbgCascade;
        if (_e233 == 0i) {
            local_1 = vec3<f32>(1f, 0.3f, 0.3f);
        } else {
            let _e235 = dbgCascade;
            if (_e235 == 1i) {
                local_2 = vec3<f32>(0.3f, 1f, 0.3f);
            } else {
                let _e237 = dbgCascade;
                local_2 = select(vec3<f32>(1f, 1f, 0.3f), vec3<f32>(0.3f, 0.3f, 1f), vec3((_e237 == 2i)));
            }
            let _e241 = local_2;
            local_1 = _e241;
        }
        ...
        let _e243 = tint;
        let _e244 = out_color;
        let _e246 = (_e244.xyz * _e243);
        out_color[0u] = _e246.x;
        out_color[1u] = _e246.y;
        out_color[2u] = _e246.z;
    }
    return;
}
