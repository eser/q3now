...
fn main_1() {
    ...
    if override_type_20_2 {
        if override_type_20_3 {
            ...
            if override_type_20_4 {
                ...
                param_10 = _e110;
                let _e111 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_8), (&param_9), (&param_10));
                base[3u] = _e111;
            } else {
                if override_type_20_5 {
                    ...
                    param_13 = _e115;
                    let _e116 = CorrectAlpha_u0028_f1_u003b_f1_u003b_vf2_u003b((&param_11), (&param_12), (&param_13));
                    base[3u] = _e116;
                }
            }
        }
    ...
    let _e128 = unnamed.lightColor;
    lightColorRadius = _e128;
    let _e129 = L_1;
    nL = normalize(_e129.xyz);
    let _e132 = V_1;
    nV = normalize(_e132.xyz);
    let _e135 = L_1;
    let _e137 = L_1;
    let _e141 = lightColorRadius[3u];
    intensFactor = (1f - (dot(_e135.xyz, _e137.xyz) * _e141));
    let _e144 = intensFactor;
    if (_e144 <= 0f) {
        discard;
    }
    let _e146 = lightColorRadius;
    let _e148 = intensFactor;
    intens = (_e146.xyz * _e148);
    dbgCascade = 0i;
    let _e150 = shadowData_1;
    param_14 = _e150.xyz;
    let _e153 = shadowData_1[3u];
    param_15 = _e153;
    let _e154 = sampleShadow_u0028_vf3_u003b_f1_u003b_i1_u003b((&param_14), (&param_15), (&param_16));
    let _e155 = param_16;
    dbgCascade = _e155;
    let _e156 = intens;
    intens = (_e156 * _e154);
    let _e158 = Np;
    let _e159 = nL;
    diffuse = dot(_e158, _e159);
    let _e161 = Np;
    let _e162 = nL;
    let _e163 = nV;
    specFactor = dot(_e161, normalize((_e162 + _e163)));
    if override_type_20_9 {
        let _e167 = diffuse;
        let _e168 = Np;
        let _e169 = nV;
        if ((_e167 * dot(_e168, _e169)) <= 0f) {
            discard;
        }
        let _e173 = diffuse;
        diffuse = abs(_e173);
        let _e175 = specFactor;
        specFactor = abs(_e175);
    } else {
        let _e177 = diffuse;
        diffuse = max(_e177, 0f);
        let _e179 = specFactor;
        specFactor = max(_e179, 0f);
    }
    let _e181 = specFactor;
    let _e185 = base;
    spec = ((vec4((pow(_e181, 10f) * 0.25f)) * _e185) * 0.8f);
    let _e188 = base;
    let _e189 = diffuse;
    let _e192 = spec;
    let _e194 = intens;
    out_color = (((_e188 * vec4(_e189)) + _e192) * vec4<f32>(_e194.x, _e194.y, _e194.z, 1f));
    let _e202 = unnamed.eyePos[3u];
    if (_e202 > 0.5f) {
        let _e204 = dbgCascade;
        if (_e204 == 0i) {
            local_1 = vec3<f32>(1f, 0.3f, 0.3f);
        } else {
            let _e206 = dbgCascade;
            if (_e206 == 1i) {
                local_2 = vec3<f32>(0.3f, 1f, 0.3f);
            } else {
                let _e208 = dbgCascade;
                local_2 = select(vec3<f32>(1f, 1f, 0.3f), vec3<f32>(0.3f, 0.3f, 1f), vec3((_e208 == 2i)));
            }
            let _e212 = local_2;
            local_1 = _e212;
        }
        ...
        let _e214 = tint;
        let _e215 = out_color;
        let _e217 = (_e215.xyz * _e214);
        out_color[0u] = _e217.x;
        out_color[1u] = _e217.y;
        out_color[2u] = _e217.z;
    }
    return;
}
