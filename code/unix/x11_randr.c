...
qboolean RandR_SetMode( int *width, int *height, int *rate )
{
	...
	{
		Com_Log( SEV_INFO, LOG_CH(ch_system), "...using desktop display mode\n" );
	...
	if ( best_fit != -1 )
	{
		//Com_Log( SEV_INFO, LOG_CH(ch_system), "...setting new mode 0x%x via xrandr \n", (int)newMode );
...
void RandR_RestoreMode( void )
{
	...
	Com_Log( SEV_INFO, LOG_CH(ch_system), "...restoring desktop display mode\n" );
...
qboolean RandR_Init( int x, int y, int w, int h )
{
	...
	if ( r_lib == NULL )
	{
		...
		if ( r_lib == NULL )
		{
			Com_Log( SEV_INFO, LOG_CH(ch_system), "...error loading libXrandr\n" );
			goto __fail;
		}
	}
	...
	for ( i = 0 ; i < ARRAY_LEN( r_list ); i++ )
	{
		*r_list[ i ].symbol = Sys_LoadFunction( r_lib, r_list[ i ].name );
		...
		{
			Com_Log( SEV_INFO, LOG_CH(ch_system), "...couldn't find '%s' in libXrandr\n", r_list[ i ].name );
			goto __fail;
		}
	}
	...
	{
		Com_Log( SEV_INFO, LOG_CH(ch_system), "...RandR extension is not available.\n" );
		goto __fail;
	}

	Com_Log( SEV_INFO, LOG_CH(ch_system), "...RandR extension version %i.%i detected.\n", ver_major, ver_minor );
