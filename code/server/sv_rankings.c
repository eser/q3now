...
*/
	...
	GR_INIT init = GRankInit( 1, SV_RankGameKey, GR_OPT_POLL, GR_OPT_END );
	s_server_context = init.context;
	s_rankings_contexts++;
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankBegin(); GR_GAMEKEY is %s\n", gamekey );
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankBegin(); s_rankings_contexts=%d\n",s_rankings_contexts );
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankBegin(); s_server_context=%d\n",init.context );
...
{
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankEnd();\n" );
	...
	for( int i = 0; i < sv_maxclients->value; i++ )
	{
		...
		{
			SV_RankUserLogout( i );
			Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankEnd: SV_RankUserLogout %d\n",i );
		}
	}
	...
	GR_STATUS status = GRankSendReportsAsync
		(
			s_server_context,
			0,
	...
	char* email )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserCreate( %d, %s, \"****\", %s );\n", index,
		username, email );

	if( !s_rankings_active )
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserCreate: Not ready to create\n" );
	...
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserCreate: Got Create from active player\n" );
		return;
	}

	// get a separate context for the new user
	GR_INIT init = GRankInit( 0, SV_RankGameKey, GR_OPT_POLL, GR_OPT_END );
	s_ranked_players[index].context = init.context;
	s_rankings_contexts++;
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserCreate(); s_rankings_contexts=%d\n",s_rankings_contexts );
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserCreate(); s_ranked_players[%d].context=%d\n",index,init.context );

	// attempt to create a new account, proceed to SV_RankUserCBF
	GR_STATUS status = GRankUserCreateAsync
		(
...
void SV_RankUserLogin( int index, char* username, char* password )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserLogin( %d, %s, \"****\" );\n", index, username );

	if( !s_rankings_active )
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserLogin: Not ready for login\n" );
		s_ranked_players[index].grank_status = QGR_STATUS_ERROR;
		return;
	}
	...
	if( s_ranked_players[index].grank_status == QGR_STATUS_ACTIVE )
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserLogin: Got Login from active player\n" );
		return;
	}
	...
	s_ranked_players[index].context = init.context;
	s_rankings_contexts++;
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserLogin(); s_rankings_contexts=%d\n",s_rankings_contexts );
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserLogin(); s_ranked_players[%d].context=%d\n",index,init.context );

	// login user, proceed to SV_RankUserCBF
	GR_STATUS status = GRankUserLoginAsync
		(
			s_ranked_players[index].context,
			username,
			password,
...
qboolean SV_RankUserValidate( int index, const char* player_id, const char* key, int token_len, int rank, char* name )
{
	...
	qboolean rVal = qfalse;

	if( !s_rankings_active )
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserValidate: Not ready to validate\n" );
		s_ranked_players[index].grank_status = QGR_STATUS_ERROR;
		return rVal;
	}

	ranked_player_t* ranked_player = &(s_ranked_players[index]);
	...
	if ( (player_id != NULL) && (key != NULL))
	{
		...
		s_rankings_contexts++;
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserValidate(); s_rankings_contexts=%d\n",s_rankings_contexts );
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserValidate(); s_ranked_players[%d].context=%d\n",index,init.context );

		// uudecode player id and player token
		ranked_player->player_id = SV_RankDecodePlayerID(player_id);
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserValidate(); ranked_player->player_id =%u\n", (uint32_t)ranked_player->player_id );
		...
		for( int i = 0; i < sv_maxclients->value; i++ )
		{
			...
			{
				Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserValidate: Duplicate login\n" );
		...
		// then validate
		GR_STATUS status = GRankPlayerValidate(
							s_server_context,
...
void SV_RankUserLogout( int index )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserLogout( %d );\n", index );
	...
	else
	{
		...
		GR_STATUS cleanup_status = GRankCleanupAsync
			(
				s_ranked_players[index].context,
				0,
				SV_RankCleanupCBF,
...
//	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankReportInt( %d, %d, %d, %d, %d );\n", index1, index2,
//		key, value, accum );
	...
	else
	{
		...
		{
			Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankReportInt: Expecting QGR_STATUS_ACTIVE"
...
//	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankReportStr( %d, %d, %d, \"%s\" );\n", index1, index2,
//		key, value );
	...
	else
	{
		if( s_ranked_players[index1].grank_status != QGR_STATUS_ACTIVE )
		{
			Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankReportStr: Unexpected status %d\n",
	...
	while( s_rankings_contexts > 1 )
	{
		...
		if( s_ranked_players != NULL )
		{
			for( int i = 0; i < sv_maxclients->value; i++ )
			{
				...
				{
					SV_RankUserLogout( i );
					Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankQuit: SV_RankUserLogout %d\n",i );
				}
				else
				{
...
static void SV_RankNewGameCBF( GR_NEWGAME* gr_newgame, void* cbf_arg )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankNewGameCBF( %08X, %08X );\n", gr_newgame, cbf_arg );
	...
	if( gr_newgame->status == GR_STATUS_OK )
	{
		...
		// start new match
		GR_MATCH match = GRankStartMatch( s_server_context );
...
static void SV_RankUserCBF( GR_LOGIN* gr_login, void* cbf_arg )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserCBF( %08X, %08X );\n", gr_login, cbf_arg );

	ranked_player_t* ranked_player = (ranked_player_t*)cbf_arg;
	...
	switch( gr_login->status )
	{
		...
		case GR_STATUS_NOUSER:
			Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserCBF: Got status %s\n",
		...
		case GR_STATUS_TIMEOUT:
			...
			break;
		default:
			Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankUserCBF: Unexpected status %s\n",
				SV_RankStatusString( gr_login->status ) );
...
static void SV_RankJoinGameCBF( GR_JOINGAME* gr_joingame, void* cbf_arg )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankJoinGameCBF( %08X, %08X );\n", gr_joingame, cbf_arg );
	...
	if( gr_joingame->status == GR_STATUS_OK )
	{
		...
		GR_MATCH match = GRankStartMatch( ranked_player->context );
...
static void SV_RankSendReportsCBF( GR_STATUS* status, void* cbf_arg )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankSendReportsCBF( %08X, %08X );\n", status, cbf_arg );

	ranked_player_t* ranked_player = (ranked_player_t*)cbf_arg;
	if( ranked_player == NULL )
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankSendReportsCBF: server\n" );
		context = s_server_context;
	}
	else
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankSendReportsCBF: player\n" );
	...
	if( context == 0 )
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankSendReportsCBF: WARNING: context == 0" );
		SV_RankCloseContext( ranked_player );
	}
	else
	{
...
static void SV_RankCleanupCBF( GR_STATUS* status, void* cbf_arg )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankCleanupCBF( %08X, %08X );\n", status, cbf_arg );
...
static void SV_RankCloseContext( ranked_player_t* ranked_player )
{
	...
	assert( s_rankings_contexts > 0 );
	s_rankings_contexts--;
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankCloseContext: s_rankings_contexts = %d\n",
		s_rankings_contexts );
	...
	int src_len )
{
	...
	for( int i = 0; i < src_len; i += 3 )
	{
		...
		int num_chars = (i + 2 < src_len) ? 4 : ((src_len - i) * 4) / 3 + 1;
	...
	int src_len )
{
	...
	for( int i = 0; i < src_len; i += 4 )
	{
		...
		// store binary data
		int num_bytes = (i + 3 < src_len) ? 3 : ((src_len - i) * 3) / 4;
	...
	int len )
{
	...
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankEncodeGameID: result buffer too small\n" );
		result[0] = '\0';
	}
	else
	{
...
static uint64_t SV_RankDecodePlayerID( const char* string )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankDecodePlayerID: string length %d\n",len );
	SV_RankAsciiDecode( buffer, string, len );
	qint64 player_id = LittleLong64(*(qint64*)buffer);
...
static void SV_RankDecodePlayerKey( const char* string, GR_PLAYER_TOKEN key )
{
	...
	int len = strlen (string) ;
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankDecodePlayerKey: string length %d\n",len );
...
static void SV_RankError( const char* fmt, ... )
{
	...
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "SV_RankError: %s\n", text );
	Com_Log( SEV_DEBUG, LOG_CH(ch_server), "****************************************\n" );
