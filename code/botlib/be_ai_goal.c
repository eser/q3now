...
// gametype_t / GT_* / GF_CAMPAIGN are now canonical in wired/protocol.h
// (transitively included via q_shared.h above).
...
// NOLINTNEXTLINE(bugprone-macro-parentheses) — `x` is a member name in `->x`; parens around it are syntactically invalid
...
static const structdef_t iteminfo_struct = {sizeof(iteminfo_t),
                                            iteminfo_fields};
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    if (spawnflags & 1) {
      ...
      if (!(AAS_PointContents(origin) & CONTENTS_WATER)) {
        ...
        trace =
            AAS_Trace(origin, ic->iteminfo[i].mins, ic->iteminfo[i].maxs, end,
...
// Changes Globals:		-
//===========================================================================
static void BotAddToAvoidGoals(bot_goalstate_t *gs, int number,
                               float avoidtime) {
  int i;
...
int BotSetupGoalAI(void) {
  ...
  g_gameflags = LibVarValue("g_gameflags", "0");
  // item configuration file
  {
    ...
    if (!itemconfig) {
      botimport.Print(PRT_WARNING,
                      "couldn't load item config, bot goals unavailable\n");
