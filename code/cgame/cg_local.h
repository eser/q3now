...
typedef struct weaponInfo_s {
	...
	float  missileTrailAnchor; // local +X distance from model origin; negative is the rear/nozzle
	float  missileDlight;
...
typedef struct {
	...
	qboolean thirdPersonCenterAimActive;
	vec3_t   thirdPersonCenterAimAngles;
	vec3_t   thirdPersonCenterAimTarget;
...
typedef struct {
	...
	// Weapon-specific layered rocket trail / classic grenade point-trail
	// classes. Scheduling and liquid handling are shared; the visual recipes
	// are deliberately independent.
	qhandle_t rocketExhaustClass;
	qhandle_t rocketSmokeClass;
	qhandle_t rocketEmberClass;
	qhandle_t grenadeTrailClass;
	...
	qhandle_t explosionShrapnelClass;
	// Shared Quake 4-authored hitscan layers. Every bullet/pellet keeps its own
	// impact origin; quality changes only the layer budget, never placement.
	qhandle_t hitscanFlashClass;
	qhandle_t hitscanMetalSparkClass;
	qhandle_t hitscanChipClass;
	qhandle_t hitscanDustClass;

	// Unified-atmosphere GPU particle recipes.  The cgame owns the semantic
	// effect composition; the renderer sees only class/profile handles.
	qhandle_t atmosphereBreathClass;
	qhandle_t atmosphereMistClass;
	qhandle_t atmosphereDebrisClass;
	...
	sfxHandle_t
		waterSplashSound; // water-surface crossing impact (Q1 misc/h2ohit1, Q3 watr_in fallback; 0 if neither present)
...
extern vmCvar_t cg_hitscanImpactDetail;
...
extern vmCvar_t
	r_pinFrameTime; // dev/visual-gate only — pin cg.time (entity-animation) for deterministic captures; sibling of r_pinShaderTime. CVAR_CHEAT, default 0.
...
#define OSP_MEMORY_CHECK( ptr )                                                          \
	if ( !( ptr ) ) {                                                                    \
		Com_Terminate( TERM_CLIENT_DROP, "%s:%d: out of memory\n", __FILE__, __LINE__ ); \
	}
...
void	 CG_ModernDrawFrame( float x, float y, float w, float h, const float *border, const float *borderColor,
							 qboolean filled );
...
void CG_AddRefEntityTemporalBase( centity_t *cent, const refEntity_t *ent, cgTemporalRole_t role );
#if FEAT_IQM
qboolean CG_ParseIQMAnimations( const char *modelName, clientInfo_t *ci );
#endif
void CG_TemporalIdentityMarkAllDiscontinuous( void );
void CG_AddRefEntityWithPowerups( centity_t *cent, refEntity_t *ent, entityState_t *state, qboolean isPlayerPart,
								  int team, cgTemporalRole_t baseRole );
...
void CG_CharacterMesh( centity_t *cent, refEntity_t *body, qhandle_t hModel, qhandle_t customShader,
					   qhandle_t customSkin, int frame, int oldframe, float backlerp, vec3_t axis[3], int renderfx,
					   int alpha, int team, refEntityMotionRole_t temporalRole );
#endif
...
void CG_MissileHitWall( int pType, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType,
		int sourceEntityNum );
...
void CG_RegisterAtmosphereParticleClasses( void ); // cg_wired_particles.c

#define CG_ATMOSPHERE_PROFILE_BREATH 60u
#define CG_ATMOSPHERE_PROFILE_MIST	 61u
#define CG_ATMOSPHERE_PROFILE_DEBRIS 62u
...
// expiry wins.
void CG_TriggerPlayerTrail( int clientNum, playerTrailType_t type, int durationMs );
...
void CG_GibPlayer( const vec3_t playerOrigin, const vec3_t playerAngles, const vec3_t playerVelocity,
				   const lerpFrame_t *bodyAnimation, const vec3_t damageDir, int knockbackParm );
...
qboolean CG_WorldToScreenPixels( vec3_t point, float *xPx,
								 float *yPx ); // WA: world -> ABSOLUTE real pixels (qfalse if behind camera)
#if FEAT_WIRED_UI
...
qhandle_t trap_R_RegisterPrimitiveShader(
	const char *
		name ); // returns all white if not found; primitive consumers (ribbon/beam) sample the shader's image by handle

// a scene is built up by calls to R_ClearScene and the various R_Add functions.
// Nothing is drawn until R_RenderScene is called.
void trap_R_ClearScene( void );
void trap_R_AddRefEntityToScene( const refEntity_t *re );
void trap_R_AddRefEntityToSceneTemporal( const refEntity_t *re, const refEntityMotion_t *motion );
...
void	 trap_R_DrawStretchPicNorm( float nx, float ny, float nw, float nh, float s1, float t1, float s2, float t2,
									qhandle_t hShader );
