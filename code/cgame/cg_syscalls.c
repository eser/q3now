...
/* Normalized-coordinate text draw: nx/ny/nSize are in 0.0-1.0 screen space */
void trap_R_DrawTextNorm( const char *text, float nx, float ny, int fontId,
	...
	syscall( CG_R_DRAWTEXTNORM, text, PASSFLOAT(nx), PASSFLOAT(ny), fontId,
	         PASSFLOAT(nSize), color, alignment, flags );
}

/* Normalized-coordinate text measure: nSize is in 0.0-1.0 screen space, returns normalized width */
float trap_R_MeasureTextNorm( const char *text, int fontId, float nSize ) {
	floatint_t fi;
	fi.i = syscall( CG_R_MEASURETEXTNORM, text, fontId, PASSFLOAT(nSize) );
	return fi.f;
}

/* Wired Store batch operations */
