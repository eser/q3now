...
vmCvar_t cg_hitscanImpactDetail;
...
static cvarTable_t cvarTable[] = {
	...
	// 1 = Quake 4 SP-authored layer counts; 0 = its compact MP recipe family.
	// Both tiers preserve every impact position and decal.
	{ &cg_hitscanImpactDetail, "cg_hitscanImpactDetail", "1", CVAR_ARCHIVE },
...
void QDECL Com_Log_Impl( log_severity_t severity, int channel, const char *msg, ... )
{
	...
	channelName = ( channel >= 0 && channel < vm_logChannelCount ) ? vm_logChannels[channel] : "";
	va_start( argptr, msg );
...
static void CG_RegisterItemSounds( int itemNum )
{
	...
	while ( *s ) {
		...
		if ( len >= MAX_QPATH || len < 5 ) {
			Com_Terminate( TERM_CLIENT_DROP, "PrecacheItem: %s has bad precache string", item->classname );
			return;
		}
	...
	if ( cgs.gametypeIsTeamGame || cg_buildScript.integer ) {
		...
		if ( cgs.gametype == GT_1FCTF || cg_buildScript.integer ) {
			cgs.media.neutralFlagReturnedSound =
				trap_S_RegisterSound( "sound/teamplay/flagreturn_opponent.opus", qtrue );
...
static void CG_RegisterGraphics( void )
{
	...
	// Native cgame modules survive map/vid restart cycles in-process. Rebuild
	// the local 1-based class table before any weapon/media registration so
	// duplicate-name rejection cannot turn every re-registered class into the
	// invalid handle 0 while the renderer keeps the same deterministic slots.
	...
	CG_RegisterRocketTrailParticleClass(); // MIG-trail-1: rocket/grenade GPU-ring classes
...
void CG_RenderViewport( int viewportKey )
{
	...
	case WUI_VIEWPORT_KEY_MAIN_SCENE:
		CG_DrawActiveFrame( fc.serverTime, (stereoFrame_t)fc.stereo, (qboolean)fc.demoPlayback );
		break;
...
void CG_Init( int serverMessageNum, int serverCommandSequence, int clientNum )
{
	...
	// version this module was built against. A mismatch is a hard error.
	{
		...
		if ( engineAbi != CGAME_IMPORT_API_VERSION ) {
			trap_Error( va( "VM ABI mismatch: module built for %d, engine %d", CGAME_IMPORT_API_VERSION, engineAbi ) );
		}
	 ...
	 * call (cl_wired_clay.c) is retired. */
	{
		...
		trap_RegisterViewportProvider( "main_scene_full", cg_main_scene_provider.lifetime,
									   cg_main_scene_provider.input_mode, cg_main_scene_provider.is_vm_routed,
									   cg_main_scene_provider.vm_key );
		trap_RegisterViewportProvider( "main_scene_split", cg_main_scene_provider.lifetime,
									   cg_main_scene_provider.input_mode, cg_main_scene_provider.is_vm_routed,
									   cg_main_scene_provider.vm_key );
	}
	 ...
	 * so it auto-cleans on map change. */
	{
		...
		trap_RegisterViewportProvider( "camera", cg_camera_provider.lifetime, cg_camera_provider.input_mode,
