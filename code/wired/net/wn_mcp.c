...
/*
====================
WN_McpHandleInitialize

MCP initialize request → capabilities response.
====================
*/
static void WN_McpHandleInitialize( wn_connection_t *conn, uint64_t stream_id, int req_id )
{
	char resp[2048];
...
/*
====================
WN_McpHandleToolsList

Return the list of available MCP tools.
====================
*/
static void WN_McpHandleToolsList( wn_connection_t *conn, uint64_t stream_id, int req_id )
{
	char resp[4096];
...
/*
====================
WN_McpHandleGameStatus

Return current game state as a JSON tool result.
====================
*/
static void WN_McpHandleGameStatus( wn_connection_t *conn, uint64_t stream_id, int req_id )
{
...
/*
====================
WN_McpHandleEventHistory

Return last N events from the ring buffer as JSON.
====================
*/
static void WN_McpHandleEventHistory( wn_connection_t *conn, uint64_t stream_id,
                                       int req_id, int count )
{
	...
	offset = Com_sprintf( resp, sizeof(resp),
		...
		(unsigned long long)( wn.event_seq > (uint64_t)count ? wn.event_seq - count : 0 ),
		(unsigned long long)wn.event_seq,
		req_id );
...
#if FEAT_BOT_IMPROVEMENTS
static void WN_McpHandleBotList( wn_connection_t *conn, uint64_t stream_id, int req_id );
static void WN_McpHandleBotGetState( wn_connection_t *conn, uint64_t stream_id, int req_id, int bot_id );
static void WN_McpHandleBotSetSkill( wn_connection_t *conn, uint64_t stream_id, int req_id, int bot_id, int skill_x10 );
#endif
                           ...
                           const byte *data, int len )
{
	...
	if ( method_len == 10 && Q_stricmpn( method, "initialize", 10 ) == 0 ) {
		WN_McpHandleInitialize( conn, stream_id, req_id );
	}
	else if ( method_len == 10 && Q_stricmpn( method, "tools/list", 10 ) == 0 ) {
		WN_McpHandleToolsList( conn, stream_id, req_id );
	}
		...
		if ( tool_len == 11 && Q_stricmpn( tool_name, "game_status", 11 ) == 0 ) {
			...
			WN_McpHandleBotSetSkill( conn, stream_id, req_id, bot_id, skill_x10 );
		}
#endif
		else {
			Com_sprintf( resp, sizeof(resp),
...
/*
====================
WN_McpHandleBotSetSkill
====================
*/
static void WN_McpHandleBotSetSkill( wn_connection_t *conn, uint64_t stream_id,
