...
static const char *WN_WeaponShortName( int mod )
{
	switch ( mod ) {
                        ...
                        const vec3_t attacker_pos, const vec3_t victim_pos )
{
	...
	mpack_write_cstr( &writer, WN_WeaponShortName( mod ) );
...
int WN_EncodeStateUpdate( byte *buf, int buf_size )
{
	...
	for ( i = 0; i < sv_maxclients->integer && i < MAX_CLIENTS; i++ ) {
		...
		mpack_write_i32( &writer, 0 ); /* stub: real value is svs.clients[i].ps.persistant[PERS_SCORE] */
		...
		mpack_write_u8( &writer, 0 ); /* stub: real value is TEAM_* from CS_PLAYERS configstring (Info_ValueForKey "t") */
