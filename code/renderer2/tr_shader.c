...
void R_RemapShader(const char *shaderName, const char *newShaderName, const char *timeOffset) {
	...
	if (sh == NULL || sh == tr.defaultShader) {
		R_LOG( rch_shaders, SEV_WARN, "WARNING: R_RemapShader: shader %s not found\n", shaderName );
		return;
	}
	...
	if (sh2 == NULL || sh2 == tr.defaultShader) {
		R_LOG( rch_shaders, SEV_WARN, "WARNING: R_RemapShader: new shader %s not found\n", newShaderName );
		return;
	}
...
static qboolean ParseStage( ComParser *parser, shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		if ( !Q_stricmp( token, "map" ) ) {
			...
			if ( !Q_stricmp( token, "$deluxemap" ) ) {
				if ( !tr.worldDeluxeMapping ) {
					R_LOG( rch_shaders, SEV_WARN, "WARNING: shader '%s' wants a deluxe map in a map compiled without them\n",
							shader.name );
					return qfalse;
				}
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( !token[0] ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameter for 'clampmap' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( token[0] == 0 ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameters for stage in shader '%s'\n", shader.name );
				continue;
			}
			...
			if ( !Q_stricmp( token, "diffuseMap" ) ) {
				...
				VectorSet4( stage->normalScale, r_baseNormalX->value, r_baseNormalY->value, 1.0f,
				...
				R_LOG( rch_shaders, SEV_WARN, "WARNING: unknown stage parameter '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( token[0] == 0 ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameter for specular reflectance in shader '%s'\n", shader.name );
				continue;
			}
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( token[0] == 0 ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameter for specular exponent in shader '%s'\n", shader.name );
				continue;
			}
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( token[0] == 0 ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameter for gloss in shader '%s'\n", shader.name );
				continue;
			}
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( token[0] == 0 ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameter for roughness in shader '%s'\n", shader.name );
				continue;
			}
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( token[0] == 0 ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameter for parallaxDepth in shader '%s'\n", shader.name );
				continue;
			}
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( token[0] == 0 ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameter for normalScale in shader '%s'\n", shader.name );
				continue;
			}
		...
		//
			...
			token = COM_ParseExt( parser, text, qfalse );
			if ( token[0] == 0 ) {
				R_LOG( rch_shaders, SEV_WARN, "WARNING: missing parameter for specularScale in shader '%s'\n", shader.name );
				continue;
			}
...
static qboolean ParseShader( ComParser *parser, const char **text )
{
	...
	while ( 1 )
	{
		...
		if ( token[0] == '{' ) {
			...
			if ( !ParseStage( parser, &stages[s], text ) ) {
				return qfalse;
			}
		...
		// tonemap parms
		if ( !Q_stricmp( token, "q3gl2_tonemap" ) ) {
			token					 = COM_ParseExt( parser, text, qfalse );
		...
		// conditional stage definition
			...
			if ( res == res_false ) {
				...
				token = COM_ParseExt( parser, text, qtrue );
				if ( token[0] == '{' )
					SkipBracedSection( parser, text, 1 );
				else
					SkipRestOfLine( parser, text );
			} else {
	...
	qboolean useLightVector, qboolean useLightVertex, qboolean parallax, qboolean tcgen)
{
	...
	//R_LOG( rch_shaders, SEV_INFO, "shader %s has diffuse %s", shader.name, diffuse->bundle[0].image[0]->imgName);
	...
	if (lightmap)
	{
		//R_LOG( rch_shaders, SEV_INFO, ", lightmap");
	...
	{
		//R_LOG( rch_shaders, SEV_INFO, ", deluxemap");
	...
	if (r_normalMapping->integer)
	{
		...
		if (normal)
		{
			//R_LOG( rch_shaders, SEV_INFO, ", normalmap %s", normal->bundle[0].image[0]->imgName);
	...
	if (r_specularMapping->integer)
	{
		...
		if (specular)
		{
			//R_LOG( rch_shaders, SEV_INFO, ", specularmap %s", specular->bundle[0].image[0]->imgName);
...
shader_t *R_FindShader( const char *name, int lightmapIndex, qboolean mipRawImage ) {
	...
	//
	{
		...
		if ( !image ) {
			...
			R_LOG( rch_shaders, SEV_DEBUG, "Couldn't find image file for shader %s\n", name );
			shader.defaultShader = qtrue;
			return FinishShader();
		}
	}
...
static void ScanAndLoadShaderFiles( void )
{
	...
	for ( i = numMtrFiles - 1; i >= 0 ; i-- ) {
		// NOLINTNEXTLINE(clang-analyzer-security.ArrayBound) — xbuffers[] sized for numMtrFiles upstream; loop index is bounded by it
		if ( xbuffers[ i ] ) {
