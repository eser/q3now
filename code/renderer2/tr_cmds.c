...
void RE_BeginFrame( stereoFrame_t stereoFrame ) {
	...
	//
	{
		static int s_measureOverdraw_mod = -1;
		int measureOverdraw_changed = ( r_measureOverdraw->modificationCount != s_measureOverdraw_mod );
		...
		if ( r_measureOverdraw->integer )
		{
			...
			s_measureOverdraw_mod = r_measureOverdraw->modificationCount;
		}
		else
		{
			// this is only reached if it was on and is now off
			if ( measureOverdraw_changed ) {
				R_IssuePendingRenderCommands();
	...
	if ( !r_ignoreGLErrors->integer )
	{
		...
		if ((err = qglGetError()) != GL_NO_ERROR)
			ri.Terminate( TERM_UNRECOVERABLE, "RE_BeginFrame() - glGetError() failed (0x%x)!", err);
	}
