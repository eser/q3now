...
void RE_LoadWorldMap( const mapFile_t *bsp );
	...
	for(i = 0; i < numLightmaps; i++)
	{
		...
		// if (tr.worldLightmapping)
		{
			...
			if (textureInternalFormat == GL_RGBA16)
			{
				...
				//R_LOG( rch_assets, SEV_INFO, "looking for %s\n", filename);
			...
			if (hdrLightmap)
			{
				...
				//R_LOG( rch_assets, SEV_INFO, "found!\n");
...
static void ParseFace( const dsurface_t *ds, const drawVert_t *verts, float *hdrVertColors, msurface_t *surf, int *indexes  ) {
	...
	if (badTriangles)
	{
		R_LOG( rch_assets, SEV_WARN, "Face has bad triangles, originally shader %s %d tris %d verts, now %d tris\n", surf->shader->name, numIndexes / 3, numVerts, numIndexes / 3 - badTriangles);
...
static void ParseTriSurf( const dsurface_t *ds, const drawVert_t *verts, float *hdrVertColors, msurface_t *surf, int *indexes ) {
	...
	if (badTriangles)
	{
		R_LOG( rch_assets, SEV_WARN, "Trisurf has bad triangles, originally shader %s %d tris %d verts, now %d tris\n", surf->shader->name, numIndexes / 3, numVerts, numIndexes / 3 - badTriangles);
		cv->numIndexes -= badTriangles * 3;
	}
...
static void R_StitchAllPatches( void ) {
	...
	while (stitched);
	R_LOG( rch_assets, SEV_DEBUG, "stitched %d LoD cracks\n", numstitches );
}
...
static	void R_LoadSurfaces( const lump_t *surfs, const lump_t *verts, const lump_t *indexLump ) {
	...
	R_LOG( rch_assets, SEV_DEBUG, "...loaded %d faces, %i meshes, %i trisurfs, %i flares\n",
...
qboolean R_GetEntityToken( char *buffer, int size ) {
	...
	// Read the entity string through the active world pointer rather than a fixed
	// storage struct — this runs post-load, when tr.world already points at the
	// world being queried.
	if ( !tr.world ) {
		buffer[0] = '\0';
		return qfalse;
...
static qboolean R_ParseSpawnVars( char *spawnVarChars, int maxSpawnVarChars, int *numSpawnVars, const char *spawnVars[MAX_SPAWN_VARS][2] )
{
	...
	*numSpawnVars = 0;
	...
	if ( com_token[0] != '{' ) {
		R_LOG( rch_assets, SEV_INFO, "R_ParseSpawnVars: found %s when expecting {\n",com_token );
		return qfalse;
	}
	...
	while ( 1 ) {
		...
		if ( !R_GetEntityToken( keyname, sizeof( keyname ) ) ) {
			R_LOG( rch_assets, SEV_INFO, "R_ParseSpawnVars: EOF without closing brace\n" );
			return qfalse;
		}
		...
		if ( com_token[0] == '}' ) {
			R_LOG( rch_assets, SEV_INFO, "R_ParseSpawnVars: closing brace without data\n" );
			return qfalse;
		}

		if ( *numSpawnVars == MAX_SPAWN_VARS ) {
			R_LOG( rch_assets, SEV_INFO, "R_ParseSpawnVars: MAX_SPAWN_VARS\n" );
			return qfalse;
		}
		...
		{
			R_LOG( rch_assets, SEV_INFO, "R_ParseSpawnVars: MAX_SPAWN_VAR_CHARS\n" );
			return qfalse;
		}
...
static void R_LoadEnvironmentJson(const char *baseName)
{
	...
	{
		R_LOG( rch_assets, SEV_INFO, "Bad %s: does not start with a object\n", filename);
	...
	if (!cubemapArrayJson)
	{
		R_LOG( rch_assets, SEV_INFO, "Bad %s: no Cubemaps\n", filename);
		ri.FS_FreeFile(buffer.v);
		return;
	}
	...
	{
		R_LOG( rch_assets, SEV_INFO, "Bad %s: Cubemaps not an array\n", filename);
		ri.FS_FreeFile(buffer.v);
		return;
	}
...
static void R_AssignCubemapsToWorldSurfaces(void)
{
	...
	for (int i = 0; i < w->numsurfaces; i++)
	{
		...
		{
			//R_LOG( rch_assets, SEV_INFO, "surface %d has no cubemap\n", i);
			continue;
		}

		surf->cubemapIndex = R_CubemapForPoint(surfOrigin);
		//R_LOG( rch_assets, SEV_INFO, "surface %d has cubemap %d\n", i, surf->cubemapIndex);
	}
}
	...
	if (0)
	{
		...
		for (i = 0; i < w->numWorldSurfaces; i++)
		{
			...
			if(ci->type & CULLINFO_PLANE)
			{
				...
				{
					//R_LOG( rch_assets, SEV_INFO, "surface %d is not oriented towards sunlight\n", i);
					continue;
				}
			}
			...
			if(ci->type & CULLINFO_BOX)
			{
				...
				/*
				R_LOG( rch_assets, SEV_INFO, "surf %d bounds (%f %f %f)-(%f %f %f) ibounds (%d %d %d)-(%d %d %d)\n", i,
				...
				*/
				...
				{
					//R_LOG( rch_assets, SEV_INFO, "surface %d is in sunlight\n", i);
					//surf->primaryLight = 1;
				}
			}
		}
