...
void CG_ModernHUDElementStatusbarValueRoutine( void *context ) {
	...
	/* Try Wired Store first, fall back to old binding */
	{
		...
		if ( storeEntry && storeEntry->text[0] ) {
			/* Dispatch 5.22 S1: an explicitly-declared .wui forecolor wins
			 * (config.color.isSet); else fall back to the store's value-driven
			 * color (default.wui legacy gradient). Mirrors statusbar_bar. */
