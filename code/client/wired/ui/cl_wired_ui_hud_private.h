...
/* every HUD-element create site routes its context
 * allocation through WiredHud_ArenaAlloc (defined in cl_wired_hud_registry.c).
 * Replaces the previous Z_Malloc / Z_Free pair with a bump-only arena that
 * is bulk-reset by WiredHud_DestroyAllElements. */
void *WiredHud_ArenaAlloc( int size );

#define ModernHUD_ELEMENT_INIT(E, CFG)                     \
    do{                                                 \
        E = WiredHud_ArenaAlloc(sizeof(*E));              \
        OSP_MEMORY_CHECK(E);                              \
        memset(E, 0, sizeof(*E));                         \
...
#define ModernHUD_MAX_OBJECTIVES  16
...
typedef struct
{
	...
	/* chat history retired: exposed through the WiredStore `hud.chat.*`
	 * prefix. Writer lives in cl_wired_ui_hud_state.c's WIRED_EVENT_CHAT /
	 * _TEAMCHAT cases; consumers walk via a `repeat` block in
	 * default.wui. ModernHUD_MAX_CHAT_LINES is the ring depth cap. */
	...
	/* Scene-caption subtitle: the localized text of the most recent caption plus
	 * the time it arrived (for the fade). Written by the WIRED_EVENT_SUBTITLE
	 * receiver, read by the scene:subtitle custom-draw, which renders on a
	 * scene-gated overlay that survives HUD-suppression (unlike the HUD msgQueue).*/
	struct {
		char subtitle[ModernHUD_MSG_MAX_LEN];   /* localized text, "" when none */
		int  arriveTime;                        /* wiredHud->time when it arrived */
	} caption;
	/* Mission objectives: the localized objective list for the hud:objectives
	 * element (a standing list, not a fade). Written by the WIRED_EVENT_OBJECTIVE
	 * receiver from the cgame-resolved text; read by the objectives element. A
	 * standard HUD element (hidden by sceneHudHidden during a cutscene, unlike the
	 * scene-exempt subtitle). */
	struct {
		char text[ModernHUD_MSG_MAX_LEN];       /* localized objective text */
		qboolean completed;
		qboolean failed;                        /* mission-fail beat: drawn struck/red */
	} objectives[ModernHUD_MAX_OBJECTIVES];
	int numObjectives;
} modernhudGlobalContext_t;

modernhudGlobalContext_t* CG_ModernHUDGetContext(void);

/* hud:subtitle element (cl_wired_hud_elem_subtitle.c). Draws the active scene-
 * caption subtitle bottom-center with a fade. Same element shape as the msgqueue;
 * the compositor exempts "hud:subtitle" from the HUD-suppress gate so it survives
 * a HUD-off cutscene. */
...
/* hud:objectives element (cl_wired_hud_elem_objectives.c). Draws the mission
 * objective list (localized text + completed/pending indicator) as a standing
 * top-left list. A STANDARD HUD element — hidden by sceneHudHidden during a
 * cutscene (NOT scene-exempt, unlike hud:subtitle). */
