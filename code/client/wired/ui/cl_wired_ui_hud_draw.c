...
//
// TRANSITION SHIM (V-13 2026-05-25): relocated from
// code/client/wired/hud/cl_wired_hud_draw.c. Retire via
// [[wiredui-hud-element-relocation]] when element files absorb the helpers
// they need and the shared helper set can either inline or move to
// elements/common.
...
void CG_ModernHUDBarPrint(const modernhudConfig_t* cfg, modernhudBarContext_t* ctx, float value)
{
	...
	modernhudDirection_t dir;

	memset(&coords, 0, sizeof(coords));

	/* The context caches direction from element-create time, when the .wui
	   `direction` keyword may not yet have been parsed into the config — leaving
	   the bar stuck at the default LEFT_TO_RIGHT. Prefer the live config value so
	   an authored `direction` (e.g. R2L for a right-anchored bar) actually applies. */
	dir = cfg->direction.isSet ? cfg->direction.value : ctx->direction;
