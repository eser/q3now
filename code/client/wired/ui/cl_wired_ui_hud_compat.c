...
/*
cl_wired_ui_hud_compat.c — Compatibility layer: syncs wired_cg/wired_cgs
from wiredHudState_t each frame.
*
* TRANSITION SHIM (V-13 2026-05-25): relocated from
* code/client/wired/hud/cl_wired_hud_compat.c. Retire in
* [[wiredui-hud-element-relocation]] when the cgame-trap compat layer
* is no longer needed.
*/
...
void WiredHud_SyncCompat( void ) {
	...
	if ( !wiredHud_state_valid ) return;
	...
	wired_cg.weaponSelectTime     = wiredHud->weaponSelectTime;
...
const char *wired_ConfigString( int index ) {
	...
	if ( !clientActiveApp->cl.gameState.stringOffsets[index] ) {
		return "";
	}
	return clientActiveApp->cl.gameState.stringData + clientActiveApp->cl.gameState.stringOffsets[index];
}
