...
// SPDX-FileCopyrightText: 2005-2006 Joerg Dietrich <dietrich_joerg@gmx.de>
...
snd_stream_t *S_OPUS_CodecOpenStream( const char *filename )
{
	...
	if ( !of )
	{
		Com_Log( SEV_DEBUG, LOG_CH(ch_sound), "S_OPUS: Failed to open %s (error %d)\n", filename, error );
	...
	// Read Opus tags for loop points
	{
		...
		if ( tags )
		{
			...
			{
				Com_Log( SEV_DEBUG, LOG_CH(ch_sound), "S_OPUS: %s has loop points: start=%d end=%d\n",
