...
void R_LoadTGA ( const char *name, byte **pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	if(length < 18)
	{
		ri.Terminate( TERM_CLIENT_DROP, "LoadTGA: header too short (%s)", name );
	}
	...
	{
		ri.Terminate( TERM_CLIENT_DROP, "LoadTGA: %s has an invalid image size", name);
	}
	...
	if ( targa_header.image_type == 2 || targa_header.image_type == 3 )
	{
		...
		{
			ri.Terminate( TERM_CLIENT_DROP, "LoadTGA: file truncated (%s)", name );
		}
		...
		switch ( targa_header.pixel_size ) {
			...
			case 32:
				...
				break;
			default:
				ri.Terminate( TERM_CLIENT_DROP, "LoadTGA: illegal pixel_size '%d' in file '%s'", targa_header.pixel_size, name );
				break;
		}
	}
