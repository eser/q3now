...
static qboolean InventoryGrow(map_inventory_t *inv) {
...
static qboolean InventoryHas(const map_inventory_t *inv,
...
static void InventoryAdd(map_inventory_t *inv,
                         asset_kind_t kind, const char *path) {
	if ( !path || !*path ) return;
...
static void ScanEntities(map_inventory_t *inv,
...
qboolean MapInventory_Build(const char *mapname,
                            ...
                            map_inventory_t *out)
{
	...
	mapFile_t *bsp = NULL;
	if ( !Map_Load( path, &bsp, MAP_LOAD_FLAG_RENDER_ONLY ) || !bsp ) {
		return qfalse;
	}
	...
	// 2. Copy entity string out before Map_Free reclaims it.
...
void MapInventory_Free(map_inventory_t *inv) {
	if ( !inv ) return;
	free( inv->entries );
