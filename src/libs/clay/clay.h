...
#include <stdbool.h>
#include <stddef.h>
#include <stdio.h> /* HANGDIAG STRIP */
...
void Clay__InitializePersistentMemory(Clay_Context* context) {
    ...
    /* q3now: upstream Clay hardcodes 10 scroll containers. Every element with a
     * `.clip` config (used by WiredUI's segmented/radio control for per-segment
     * and label text truncation, plus flex scroll panels) consumes one slot, so
     * a settings page with several segmented rows exceeds 10 and Clay raises an
     * out-of-bounds internal error that corrupts the offending control's render.
     * Size it like the other per-layout arrays (maxElementCount) so clip-based
     * truncation scales with element count. Arena cost is ~100 B/slot. */
...
//                    && (xAxis || !Clay__ElementHasConfig(childElement, CLAY__ELEMENT_CONFIG_TYPE_ASPECT))
            ...
            if (sizingAlongAxis) {
                ...
                if (sizeToDistribute < 0) {
                    ...
                    int zz_guard1 = 0; /* HANGDIAG STRIP */
                    while (sizeToDistribute < -CLAY__EPSILON && resizableContainerBuffer.length > 0) {
                        if (++zz_guard1 > 100000) { fprintf(stderr, "[HANGDIAG] SHRINK loop runaway sizeToDistribute=%f bufLen=%d\n", sizeToDistribute, resizableContainerBuffer.length); break; } /* HANGDIAG STRIP */
                        float largest = 0;
                ...
                // The content is too small, allow SIZING_GROW containers to expand
                    ...
                    int zz_guard2 = 0; /* HANGDIAG STRIP */
                    while (sizeToDistribute > CLAY__EPSILON && resizableContainerBuffer.length > 0) {
                        if (++zz_guard2 > 100000) { fprintf(stderr, "[HANGDIAG] GROW loop runaway sizeToDistribute=%f bufLen=%d\n", sizeToDistribute, resizableContainerBuffer.length); break; } /* HANGDIAG STRIP */
...
void Clay__CalculateFinalLayout(void) {
    ...
    int zz_dfs1 = 0; /* HANGDIAG STRIP */
    while (dfsBuffer.length > 0) {
        if (++zz_dfs1 > 2000000) { fprintf(stderr, "[HANGDIAG] DFS1(sizing) runaway dfsLen=%d\n", dfsBuffer.length); break; } /* HANGDIAG STRIP */
    ...
    for (int32_t rootIndex = 0; rootIndex < context->layoutElementTreeRoots.length; ++rootIndex) {
        ...
        int zz_dfs2 = 0; /* HANGDIAG STRIP */
        while (dfsBuffer.length > 0) {
            if (++zz_dfs2 > 2000000) { fprintf(stderr, "[HANGDIAG] DFS2(position) runaway dfsLen=%d\n", dfsBuffer.length); break; } /* HANGDIAG STRIP */
            Clay__LayoutElementTreeNode *currentElementTreeNode = Clay__LayoutElementTreeNodeArray_Get(&dfsBuffer, (int)dfsBuffer.length - 1);
            Clay_LayoutElement *currentElement = currentElementTreeNode->layoutElement;
            Clay_LayoutConfig *layoutConfig = currentElement->layoutConfig;
...
Clay__RenderDebugLayoutData Clay__RenderDebugLayoutElementsList(int32_t initialRootsLength, int32_t highlightedRowIndex) {
    ...
    for (int32_t rootIndex = 0; rootIndex < initialRootsLength; ++rootIndex) {
        ...
        int zz_dfs3 = 0; /* HANGDIAG STRIP */
        while (dfsBuffer.length > 0) {
            if (++zz_dfs3 > 2000000) { fprintf(stderr, "[HANGDIAG] DFS3 runaway dfsLen=%d\n", dfsBuffer.length); break; } /* HANGDIAG STRIP */
...
void Clay_SetPointerState(Clay_Vector2 position, bool isPointerDown) {
    ...
    for (int32_t rootIndex = context->layoutElementTreeRoots.length - 1; rootIndex >= 0; --rootIndex) {
        ...
        context->treeNodeVisited.internalArray[0] = false;
        bool found = false;
        int zz_dfs4 = 0; /* HANGDIAG STRIP */
        while (dfsBuffer.length > 0) {
            if (++zz_dfs4 > 2000000) { fprintf(stderr, "[HANGDIAG] DFS4(render) runaway dfsLen=%d\n", dfsBuffer.length); break; } /* HANGDIAG STRIP */
            if (context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
                dfsBuffer.length--;
                continue;
            }
