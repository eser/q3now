...
cd `dirname $0`
if [ ! -f Makefile ]; then
	echo "This script must be run from the wired build directory"
	exit 1
fi
...
if [ "$1" == "notarize" ]; then
	...
	POST_NOTARIZED_ZIP="wired_notarized.zip"

	BUNDLE_ID="com.eser.wired"
	...
	ENTITLEMENTS_FILE="misc/macos/wired.entitlements"
